...
struct SuggestionsState {
  ...
  void pushLocals() {
    for (int i = ls->fs->nactvar - 1; i >= 0; i--) {
      ...
      if (strcmp(vd->vd.name->contents, "obsid_operator_new") != 0
          && strcmp(vd->vd.name->contents, "obsid_operator_extends") != 0
          && strcmp(vd->vd.name->contents, "obsid_operator_instanceof") != 0
        ) {
