...
static luaL_Reg clang_functions[] = {{"createIndex", l_createIndex},
                                     {NULL, NULL}};
...
static int l_parseTU(lua_State *L) {
  ...
  CXTranslationUnit *tu = newTU(L);
  *tu = clang_parseTranslationUnit(idx, sourceFile, args, nArgs, NULL, 0,
...
static luaL_Reg index_functions[] = {{"load", l_loadTU},
                                     {"parse", l_parseTU},
                                     {"__gc", l_indexGc},
                                     {NULL, NULL}};
...
static luaL_Reg tu_functions[] = {{"cursor", l_cursor},
                                  {"file", l_file},
...
enum CXChildVisitResult LuaTableVisitor(CXCursor cursor, CXCursor parent,
...
static int l_parent(lua_State *L) {
  ...
  *parent = clang_getCursorSemanticParent(cur);
  if (clang_Cursor_isNull(*parent)) lua_pushnil(L);
  return 1;
}
...
static int l_type(lua_State *L) {
  ...
  *type = clang_getCursorType(cur);
  if (type->kind == CXType_Invalid) lua_pushnil(L);
  return 1;
}
...
static int l_referenced(lua_State *L) {
  ...
  *res = clang_getCursorReferenced(cur);
  if (clang_Cursor_isNull(*res)) lua_pushnil(L);
  return 1;
}
...
static luaL_Reg cursor_functions[] = {{"children", l_children},
                                      {"kind", l_kind},
                                      {"name", l_name},
                                      {"__tostring", l_name},
...
static int l_canonical(lua_State *L) {
  ...
  if (can->kind == CXType_Invalid) lua_pushnil(L);
  return 1;
}
...
static int l_declaration(lua_State *L) {
  ...
  *cur = clang_getTypeDeclaration(type);
  if (clang_Cursor_isNull(*cur)) lua_pushnil(L);
  return 1;
}
...
static int l_pointee(lua_State *L) {
  ...
  *res = clang_getPointeeType(type);
  if (res->kind == CXType_Invalid) lua_pushnil(L);
  return 1;
}
...
static luaL_Reg type_functions[] = {{"__tostring", l_typeToString},
                                    {"name", l_typeToString},
                                    {"spelling", l_spelling},
...
extern "C" EXPORT_API int luaopen_clang(lua_State *L) {
