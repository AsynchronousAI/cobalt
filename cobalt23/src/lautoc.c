...
void luaA_open(lua_State* L) {
  ...
  luaA_conversion_type(L, luaA_type_add(L, "bool", sizeof(bool)),
                       luaA_push_bool, luaA_to_bool);
  luaA_conversion_type(L, luaA_type_add(L, "_Bool", sizeof(bool)),
                       luaA_push_bool, luaA_to_bool);
  ...
  luaA_conversion(L, unsigned char, luaA_push_unsigned_char,
                  luaA_to_unsigned_char);
  luaA_conversion(L, short, luaA_push_short, luaA_to_short);
  luaA_conversion(L, unsigned short, luaA_push_unsigned_short,
                  luaA_to_unsigned_short);
  luaA_conversion(L, int, luaA_push_int, luaA_to_int);
  luaA_conversion(L, unsigned int, luaA_push_unsigned_int,
                  luaA_to_unsigned_int);
  luaA_conversion(L, long, luaA_push_long, luaA_to_long);
  luaA_conversion(L, unsigned long, luaA_push_unsigned_long,
                  luaA_to_unsigned_long);
  ...
  luaA_conversion_push_type(L, luaA_type_add(L, "const bool", sizeof(bool)),
                            luaA_push_bool);
  luaA_conversion_push_type(L, luaA_type_add(L, "const _Bool", sizeof(bool)),
                            luaA_push_bool);
  ...
  luaA_conversion_push(L, const unsigned long long,
                       luaA_push_unsigned_long_long);
  ...
  luaA_conversion(L, const char*, luaA_push_const_char_ptr,
                  luaA_to_const_char_ptr);
...
const char* luaA_typename(lua_State* L, luaA_Type id) {
  ...
  const char* type =
      lua_isnil(L, -1) ? "LUAA_INVALID_TYPE" : lua_tostring(L, -1);
  lua_pop(L, 2);
...
int luaA_push_type(lua_State* L, luaA_Type type_id, const void* c_in) {
  ...
  lua_pushfstring(
      L, "luaA_push: conversion to Lua object from type '%s' not registered!",
...
void luaA_to_type(lua_State* L, luaA_Type type_id, void* c_out, int index) {
  ...
  lua_pushfstring(
      L, "luaA_to: conversion from Lua object to type '%s' not registered!",
      luaA_typename(L, type_id));
  lua_error(L);
}
...
void luaA_conversion_to_type(lua_State* L, luaA_Type type_id,
                             luaA_Tofunc func) {
  lua_getfield(L, LUA_REGISTRYINDEX, LUAA_REGISTRYPREFIX "stack_to");
  lua_pushinteger(L, type_id);
  lua_pushlightuserdata(L, func);
...
void luaA_to_signed_char(lua_State* L, luaA_Type type_id, void* c_out,
                         int index) {
...
void luaA_to_unsigned_char(lua_State* L, luaA_Type type_id, void* c_out,
                           int index) {
...
int luaA_push_unsigned_short(lua_State* L, luaA_Type type_id,
                             const void* c_in) {
  lua_pushinteger(L, *(unsigned short*)c_in);
  return 1;
}

void luaA_to_unsigned_short(lua_State* L, luaA_Type type_id, void* c_out,
                            int index) {
...
void luaA_to_unsigned_int(lua_State* L, luaA_Type type_id, void* c_out,
                          int index) {
...
void luaA_to_unsigned_long(lua_State* L, luaA_Type type_id, void* c_out,
                           int index) {
...
void luaA_to_long_long(lua_State* L, luaA_Type type_id, void* c_out,
                       int index) {
  *(long long*)c_out = lua_tointeger(L, index);
}

int luaA_push_unsigned_long_long(lua_State* L, luaA_Type type_id,
                                 const void* c_in) {
  lua_pushinteger(L, *(unsigned long long*)c_in);
  return 1;
}

void luaA_to_unsigned_long_long(lua_State* L, luaA_Type type_id, void* c_out,
                                int index) {
...
void luaA_to_long_double(lua_State* L, luaA_Type type_id, void* c_out,
                         int index) {
...
int luaA_push_const_char_ptr(lua_State* L, luaA_Type type_id,
                             const void* c_in) {
  lua_pushstring(L, *(const char**)c_in);
  return 1;
}

void luaA_to_const_char_ptr(lua_State* L, luaA_Type type_id, void* c_out,
                            int index) {
...
int luaA_struct_push_member_offset_type(lua_State* L, luaA_Type type,
  ...
  if (!lua_isnil(L, -1)) {
    ...
    lua_pop(L, 3);
    lua_pushfstring(L,
                    "luaA_struct_push_member: Member offset '%d' not "
                    "registered for struct '%s'!",
...
int luaA_struct_push_member_name_type(lua_State* L, luaA_Type type,
  ...
  if (!lua_isnil(L, -1)) {
    ...
    lua_pop(L, 3);
    lua_pushfstring(L,
                    "luaA_struct_push_member: Member name '%s' not registered "
                    "for struct '%s'!",
...
void luaA_struct_to_member_offset_type(lua_State* L, luaA_Type type,
  ...
  if (!lua_isnil(L, -1)) {
    ...
    lua_pop(L, 3);
    lua_pushfstring(L,
                    "luaA_struct_to_member: Member offset '%d' not registered "
                    "for struct '%s'!",
                    offset, luaA_typename(L, type));
    lua_error(L);
  }
...
void luaA_struct_to_member_name_type(lua_State* L, luaA_Type type,
  ...
  if (!lua_isnil(L, -1)) {
    ...
    lua_pop(L, 3);
    lua_pushfstring(L,
                    "luaA_struct_to_member: Member name '%s' not registered "
                    "for struct '%s'!",
                    member, luaA_typename(L, type));
    lua_error(L);
  }
...
luaA_Type luaA_struct_typeof_member_offset_type(lua_State* L, luaA_Type type,
  ...
  if (!lua_isnil(L, -1)) {
    ...
    lua_pop(L, 3);
    lua_pushfstring(L,
                    "luaA_struct_typeof_member: Member offset '%d' not "
                    "registered for struct '%s'!",
                    offset, luaA_typename(L, type));
    lua_error(L);
  }
...
luaA_Type luaA_struct_typeof_member_name_type(lua_State* L, luaA_Type type,
  ...
  if (!lua_isnil(L, -1)) {
    ...
    lua_pop(L, 3);
    lua_pushfstring(L,
                    "luaA_struct_typeof_member: Member name '%s' not "
                    "registered for struct '%s'!",
                    member, luaA_typename(L, type));
    lua_error(L);
  }
...
int luaA_struct_push_type(lua_State* L, luaA_Type type, const void* c_in) {
  ...
  lua_pop(L, 2);
  lua_pushfstring(L, "lua_struct_push: Struct '%s' not registered!",
                  luaA_typename(L, type));
  lua_error(L);
  return 0;
}
...
int luaA_enum_push_type(lua_State* L, luaA_Type type, const void* value) {
  ...
  lua_pop(L, 2);
  lua_pushfstring(L, "luaA_enum_push: Enum '%s' not registered!",
                  luaA_typename(L, type));
  lua_error(L);
  return 0;
}
...
void luaA_enum_to_type(lua_State* L, luaA_Type type, void* c_out, int index) {
  ...
  lua_pop(L, 3);
  lua_pushfstring(L, "luaA_enum_to: Enum '%s' not registered!",
                  luaA_typename(L, type));
  lua_error(L);
  return;
}
...
bool luaA_enum_has_name_type(lua_State* L, luaA_Type type, const char* name) {
  ...
  lua_pop(L, 2);
  lua_pushfstring(L, "luaA_enum_has_name: Enum '%s' not registered!",
                  luaA_typename(L, type));
  lua_error(L);
  return false;
}
...
void luaA_enum_value_type(lua_State* L, luaA_Type type, const void* value,
  ...
  lua_pop(L, 2);
  lua_pushfstring(L, "luaA_enum_value: Enum '%s' not registered!",
                  luaA_typename(L, type));
  lua_error(L);
}
...
int luaA_call_entry(lua_State* L) {
  /* Get return size */
...
int luaA_call(lua_State* L, void* func_ptr) {
  ...
  lua_pop(L, 1);
  lua_pushfstring(L, "luaA_call: Function with address '%p' is not registered!",
                  func_ptr);
  lua_error(L);
  return 0;
}
...
int luaA_call_name(lua_State* L, const char* func_name) {
  ...
  lua_pop(L, 1);
  lua_pushfstring(L, "luaA_call_name: Function '%s' is not registered!",
                  func_name);
  lua_error(L);
  return 0;
}
