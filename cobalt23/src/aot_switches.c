...
static void println_goto_ret_s() {
...
static void create_function_switches(Proto *f) {
  ...
  println("static");
  println("CallInfo *magic_implementation_%02d(lua_State *L, CallInfo *ci)",
          func_id);
  println("{");
  println("  LClosure *cl;");
  println("  TValue *k;");
  ...
  for (int pc = 0; pc < f->sizecode; pc++) {
    ...
    switch (op) {
      ...
      case OP_GETTABLE: {
        ...
        println(
            "            ? (cast_void(n = ivalue(rc)), luaV_fastgeti(L, rb, n, "
            "slot))");
      ...
      case OP_SETTABLE: {
        ...
        println(
            "            ? (cast_void(n = ivalue(rb)), luaV_fastgeti(L, "
            "s2v(ra), n, slot))");
        println(
      ...
      case OP_SETFIELD: {
        ...
        println(
            "        TString *key = tsvalue(rb);  /* key must be a string */");
        println(
            "        if (luaV_fastget(L, s2v(ra), key, slot, "
            "luaH_getshortstr)) {");
        println("          luaV_finishfastset(L, s2v(ra), slot, rc);");
        println("        }");
        println("        else");
      ...
      case OP_NEWTABLE: {
        ...
        println("          c += GETARG_Ax(0x%08x) * (MAXARG_C + 1);",
                f->code[pc + 1]);
      ...
      case OP_MMBIN: {
        println(
            "        Instruction pi = 0x%08x; /* original arith. expression */",
            f->code[pc - 1]);
        println("        TValue *rb = vRB(i);");
        println("        TMS tm = (TMS)GETARG_C(i);");
        println("        StkId result = RA(pi);");
        println(
            "        lua_assert(OP_ADD <= GET_OPCODE(pi) && GET_OPCODE(pi) <= "
            "OP_SHR);");
      ...
      case OP_MMBINI: {
        println(
            "        Instruction pi = 0x%0x;  /* original arith. expression */",
            f->code[pc - 1]);
        ...
        println("        StkId result = RA(pi);");
        println(
            "        Protect(luaT_trybiniTM(L, s2v(ra), imm, flip, result, "
            "tm));");
        // FALLTHROUGH
        break;
      }
      case OP_MMBINK: {
        ...
        println("        StkId result = RA(pi);");
        println(
            "        Protect(luaT_trybinassocTM(L, s2v(ra), imm, flip, result, "
            "tm));");
        // FALLTHROUGH
        break;
      }
      ...
      case OP_CONCAT: {
        println(
            "        int n = GETARG_B(i);  /* number of elements to "
            "concatenate */");
        println(
      ...
      case OP_TAILCALL: {
        ...
        println(
            "        /* delta is virtual 'func' - real 'func' (vararg "
            "functions) */");
        println(
            "        int delta = (nparams1) ? ci->u.l.nextraargs + nparams1 : "
            "0;");
        println("        if (b != 0)");
        println("          L->top = ra + b;");
        println("        else  /* previous instruction set top */");
        ...
        println("        if (TESTARG_k(i)) {");
        println(
            "          luaF_closeupval(L, base);  /* close upvalues from "
            "current call */");
        println(
            "          lua_assert(L->tbclist < base);  /* no pending tbc "
            "variables */");
        ...
        println(
            "          luaD_poscall(L, ci, cast_int(L->top - ra));  /* finish "
            "caller */");
        println(
      ...
      case OP_RETURN0: {
        ...
        println(
            "          for (nres = ci->nresults; l_unlikely(nres > 0); "
            "nres--)");
        println(
      ...
      case OP_TFORCALL: {
        println(
            "        /* 'ra' has the iterator function, 'ra + 1' has the "
            "state,");
        println(
            "           'ra + 2' has the control variable, and 'ra + 3' has "
            "the");
        println(
            "           to-be-closed variable. The call will use the stack "
            "after");
        ...
        println(
            "        ProtectNT(luaD_call(L, ra + 4, GETARG_C(i)));  /* do the "
            "call */");
      ...
      case OP_SETLIST: {
        // We should take care to only generate code for the extra argument if
        // it actually exists. In a previous version of this compiler, we were
        // putting the "if" in the generated code instead of in the code
        // generator and that resulted in compile-time warnings from gcc.
        // Sometimes, the last += GETARG_Ax" line would overflow and the C
        // compiler would naturally complain (even though the offending line was
        // never executed).
        ...
        println("        else");
        println(
            "          L->top = ci->top;  /* correct top in case of emergency "
            "GC */");
        println("        last += n;");
        if (has_extra_arg) {
          println("        last += GETARG_Ax(0x%08x) * (MAXARG_C + 1);",
                  f->code[pc + 1]);
          println("        pc++;");
        }
        println(
        ...
        println(
            "          luaH_resizearray(L, h, last);  /* preallocate it at "
            "once */");
        println("        for (; n > 0; n--) {");
      ...
      case OP_VARARGPREP: {
        println(
            "        ProtectNT(luaT_adjustvarargs(L, GETARG_A(i), ci, "
            "cl->p));");
        println(
            "        if (l_unlikely(trap)) {  /* previous \"Protect\" updated "
            "trap */");
        println("          luaD_hookcall(L, ci);");
        println(
            "          L->oldpc = 1;  /* next opcode will be seen as a \"new\" "
            "line */");
        println("        }");
        println(
            "        updatebase(ci);  /* function has new base after "
            "adjustment */");
        // FALLTHROUGH
        break;
      }
