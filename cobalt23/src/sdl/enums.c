...
static rec_t *code_search(int domain, int code) {
  ...
  tmp.domain = domain;
  tmp.code = code;
  return RB_FIND(CodeTree, &CodeHead, &tmp);
}
static rec_t *code_first(int domain, int code) {
  rec_t tmp;
  tmp.domain = domain;
  tmp.code = code;
  return RB_NFIND(CodeTree, &CodeHead, &tmp);
}
...
static rec_t *str_search(int domain, const char *str) {
  ...
  tmp.domain = domain;
  tmp.str = (char *)str;
  return RB_FIND(StringTree, &StringHead, &tmp);
}
#if 0
...
static void enums_free(lua_State *L, rec_t *rec) {
  if (code_search(rec->domain, rec->code) == rec) code_remove(rec);
  if (str_search(rec->domain, rec->str) == rec) str_remove(rec);
  Free(L, rec->str);
  Free(L, rec);
}
...
void enums_free_all(lua_State *L) {
  rec_t *rec;
  while ((rec = code_first(0, 0))) enums_free(L, rec);
}
...
#define CASE(xxx) \
  if (strcmp(s, "" #xxx) == 0) return values##xxx(L)
  CASE(hintpriority);
  CASE(logpriority);
  CASE(pixelformat);
  CASE(eventtype);
...
#define ADD(what, s)                                       \
  ...
  ADD(CONTROLLER_TYPE_NINTENDO_SWITCH_JOYCON_LEFT,
      "nintendo switch joycon left");
  ADD(CONTROLLER_TYPE_NINTENDO_SWITCH_JOYCON_RIGHT,
      "nintendo switch joycon right");
  ADD(CONTROLLER_TYPE_NINTENDO_SWITCH_JOYCON_PAIR,
