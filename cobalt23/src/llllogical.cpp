...
Logical::Logical(CompilerState& cs, Stack& stack)
  ...
  assert(GET_OPCODE(cs.instr_) == OP_BAND || GET_OPCODE(cs.instr_) == OP_BOR ||
         GET_OPCODE(cs.instr_) == OP_BXOR || GET_OPCODE(cs.instr_) == OP_SHL ||
...
void Logical::ComputeTaggedMethod() {
  ...
  auto args = {cs_.values_.state, rkb_.GetTValue(), rkc_.GetTValue(),
