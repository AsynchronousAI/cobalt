...
static void applyextends (LexState *ls, expdesc *v, TString *parent, int line) {
  ...
  expdesc f;
  singlevaraux(fs, luaS_newliteral(ls->L, "obsid_operator_extends"), &f, 1);
...
static void newexpr (LexState *ls, expdesc *v) {
  ...
  singlevaraux(fs, luaS_newliteral(ls->L, "obsid_operator_new"), v, 1);
  lua_assert(v->k != VVOID);
  luaK_exp2nextreg(fs, v);
...
static void instanceof (LexState *ls, expdesc *v) {
  ...
  singlevaraux(fs, luaS_newliteral(ls->L, "obsid_operator_instanceof"), v, 1);
  lua_assert(v->k != VVOID);
  luaK_exp2nextreg(fs, v);
...
static void switchstat (LexState *ls, int line) {
  ...
  TString* const begin_switch = luaS_newliteral(ls->L, "obsid_begin_switch");
  TString* const end_switch = luaS_newliteral(ls->L, "obsid_end_switch");
  ...
  while (gett(ls) != TK_END) {
    ...
    if (gett(ls) == TK_DEFAULT) {
      ...
      default_case = luaS_newliteral(ls->L, "obsid_default_case");
...
static void statement (LexState *ls, TypeHint *prop) {
  if (ls->shouldSuggest()) {
    ...
    ss.push("stat", "function");
    ss.push("stat", "class");
    ss.push("stat", "obsid_class");
    ss.push("stat", "local");
    ss.push("stat", "export");
    ss.push("stat", "obsid_export");
    ss.push("stat", "return");
    ss.push("stat", "break");
    ss.push("stat", "continue");
    ss.push("stat", "goto");
    ss.push("stat", "switch");
    ss.push("stat", "obsid_switch");
    ss.push("stat", "enum");
    ss.push("stat", "obsid_use");
    ss.push("stat", "new");
    ss.push("stat", "obsid_new");
...
static void builtinoperators (LexState *ls) {
  ...
  if (uses_new || uses_extends || uses_instanceof) {
    ...
    if (uses_new) {
      ...
      ls->tokens.emplace_back(Token(TK_NAME, luaX_newliteral(ls, "obsid_operator_new")));
    ...
    if (uses_extends) {
      ...
      ls->tokens.emplace_back(Token(TK_LOCAL));
      ls->tokens.emplace_back(Token(TK_FUNCTION));
      ls->tokens.emplace_back(Token(TK_NAME, luaX_newliteral(ls, "obsid_operator_extends")));
      ls->tokens.emplace_back(Token('('));
    ...
    if (uses_instanceof) {
      ...
      ls->tokens.emplace_back(Token(TK_LOCAL));
      ls->tokens.emplace_back(Token(TK_FUNCTION));
      ls->tokens.emplace_back(Token(TK_NAME, luaX_newliteral(ls, "obsid_operator_instanceof")));
      ls->tokens.emplace_back(Token('('));
      ls->tokens.emplace_back(Token(TK_NAME, luaX_newliteral(ls, "t")));
      ls->tokens.emplace_back(Token(','));
      ls->tokens.emplace_back(Token(TK_NAME, luaX_newliteral(ls, "mt")));
