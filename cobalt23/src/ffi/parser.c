...
#define IS_INLINE(tok)                                         \
  (IS_LITERAL(tok, "inline") || IS_LITERAL(tok, "__inline") || \
   IS_LITERAL(tok, "__inline__"))
...
static int next_token(lua_State* L, struct parser* P, struct token* tok) {
  ...
  /*fprintf(stderr, "token %d %d %.*s %.10s\n", tok->type, (int) tok->size,
   * (tok->type == TOK_TOKEN || tok->type == TOK_STRING) ? (int) tok->size : 0,
   * tok->str, P->next);*/
  return 1;
}
...
static void require_token_line(lua_State* L, struct parser* P,
                               struct token* tok, const char* file, int line) {
  if (!next_token(L, P, tok)) {
    luaL_error(L, "unexpected end on line %s:%d", file, line);
  }
}

static void check_token(lua_State* L, struct parser* P, int type,
...
static void calculate_member_position(lua_State* L, struct parser* P,
                                      struct ctype* ct, struct ctype* mt,
                                      int* pbit_offset, int* pbitfield_type) {
  ...
  if (ct->type == UNION_TYPE) {
    ...
    if (mt->has_member_name && mt->bit_size == 0) {
      luaL_error(L, "zero length bitfields must be unnamed on line %d",
                 P->line);
    }
    ...
    *pbitfield_type = mt->align_mask;
    ...
    //          * bitfields never add any padding, except for :0 which still
    //          forces
...
static int copy_submembers(lua_State* L, int to_usr, int from_usr,
                           const struct ctype* ft, int* midx) {
  struct ctype ct;
  int i, sublen;
...
static int parse_record(lua_State* L, struct parser* P, struct ctype* ct) {
  ...
  if (tok.type == TOK_TOKEN) {
    ...
    if (lua_isnil(L, -1)) {
      ...
      const struct ctype* prevt =
          (const struct ctype*)lua_touserdata(L, top + 3);
...
#define LONG_TYPE(u) \
  (sizeof(long) == sizeof(int64_t) ? u "int64_t" : u "int32_t")
    if (flags & UNSIGNED) {
...
static int parse_attribute(lua_State* L, struct parser* P, struct token* tok,
  ...
  if (tok->type != TOK_TOKEN) {
    return 0;
             ...
             (IS_LITERAL(*tok, "__asm__") || IS_LITERAL(*tok, "__asm"))) {
...
int parse_type(lua_State* L, struct parser* P, struct ctype* ct) {
  ...
  for (;;) {
    if (tok.type != TOK_TOKEN) {
      return luaL_error(L, "unexpected value before type name on line %d",
                        P->line);
      ...
      require_token(L, P, &tok);
               ...
               IS_LITERAL(tok, "static") || IS_INLINE(tok)) {
      /* ignored for now */
      require_token(L, P, &tok);
...
static void append_type_name(luaL_Buffer* B, int usr, const struct ctype* ct,
  ...
  if (type == FRONT || type == BOTH) {
    if (ct->type != FUNCTION_PTR_TYPE &&
        (ct->const_mask & (1 << ct->pointers))) {
...
static void push_function_type_strings(lua_State* L, int usr,
                                       const struct ctype* ct) {
  size_t i, args;
  luaL_Buffer B;
  int top = lua_gettop(L);
  ...
  for (i = 1; i <= args; i++) {
    ...
    append_type_name(&B, arg_usr,
                     (const struct ctype*)lua_touserdata(L, arg_ct), BOTH);
  }
  ...
  for (;;) {
    ...
    if (args) {
      if (tok.type != TOK_COMMA) {
        luaL_error(L, "unexpected token in function argument %d on line %d",
                   args, P->line);
      }
    ...
    if (tok.type == TOK_VA_ARG) {
      ...
      /* check for the c style int func(void) and error on other uses of
...
static struct ctype* parse_argument2(lua_State* L, struct parser* P, int ct_usr,
  ...
  for (;;) {
    if (!next_token(L, P, &tok)) {
      ...
      if (!ct->is_packed) {
        ct->align_mask =
            max(min(PTR_ALIGN_MASK, P->align_mask), ct->align_mask);
      }
...
static void find_canonical_usr(lua_State* L, int ct_usr,
                               const struct ctype* ct) {
  struct ctype rt;
  int top = lua_gettop(L);
  int types;
 ...
 */
void parse_argument(lua_State* L, struct parser* P, int ct_usr,
                    struct ctype* ct, struct token* pname,
                    struct parser* asmname) {
...
static void parse_typedef(lua_State* L, struct parser* P) {
  ...
  for (;;) {
    ...
    if (!name.size) {
      ...
      luaL_error(L, "Can't typedef a variable length array on line %d",
                 P->line);
    }
...
static void parse_constant_assignemnt(lua_State* L, struct parser* P,
                                      const struct ctype* type,
                                      const struct token* name) {
  int64_t val = calculate_constant(L, P);

  check_token(L, P, TOK_SEMICOLON, "",
              "expected ; after constant definition on line %d", P->line);
...
static int64_t calculate_constant2(lua_State* L, struct parser* P,
                                   struct token* tok);

/* () */
static int64_t calculate_constant1(lua_State* L, struct parser* P,
                                   struct token* tok) {
  int64_t ret;
...
static int64_t calculate_constant3(lua_State* L, struct parser* P,
                                   struct token* tok) {
...
/* binary + and - (left associative) */
static int64_t calculate_constant4(lua_State* L, struct parser* P,
                                   struct token* tok) {
...
/* binary << and >> (left associative) */
static int64_t calculate_constant5(lua_State* L, struct parser* P,
                                   struct token* tok) {
...
/* binary <, <=, >, and >= (left associative) */
static int64_t calculate_constant6(lua_State* L, struct parser* P,
                                   struct token* tok) {
...
static int64_t calculate_constant7(lua_State* L, struct parser* P,
                                   struct token* tok) {
...
/* binary & (left associative) */
static int64_t calculate_constant8(lua_State* L, struct parser* P,
                                   struct token* tok) {
...
/* binary ^ (left associative) */
static int64_t calculate_constant9(lua_State* L, struct parser* P,
                                   struct token* tok) {
...
/* binary | (left associative) */
static int64_t calculate_constant10(lua_State* L, struct parser* P,
                                    struct token* tok) {
...
/* binary && (left associative) */
static int64_t calculate_constant11(lua_State* L, struct parser* P,
                                    struct token* tok) {
...
/* binary || (left associative) */
static int64_t calculate_constant12(lua_State* L, struct parser* P,
                                    struct token* tok) {
...
static int64_t calculate_constant13(lua_State* L, struct parser* P,
                                    struct token* tok) {
