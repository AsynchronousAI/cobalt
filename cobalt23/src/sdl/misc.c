...
static int EncloseFPoints(lua_State *L) {
  ...
  rc = SDL_EncloseFPoints(points, count, (ec == ERR_NOTPRESENT) ? NULL : &clip,
                          &result);
  Free(L, points);
  lua_pushboolean(L, rc);
  pushfrect(L, &result);
...
static int IntersectFRectAndLine(lua_State *L) {
  ...
  if (lua_istable(L, 2)) {
    ...
    if (!SDL_IntersectFRectAndLine(&r, &point1.x, &point1.y, &point2.x,
                                   &point2.y))
      return 0;
    pushfpoint(L, &point1);
    pushfpoint(L, &point2);
...
void moonsdl2_open_misc(lua_State *L) { luaL_setfuncs(L, Functions, 0); }
