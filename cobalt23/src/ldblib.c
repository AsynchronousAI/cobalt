...
static void mark_object(lua_State *L, lua_State *dL, const void *parent,
                        const char *desc);
...
static const void *readobject(lua_State *L, lua_State *dL, const void *parent,
                              const char *desc) {
  int t = lua_type(L, -1);
  int tidx = 0;
  switch (t) {
...
static const char *keystring(lua_State *L, int index, char *buffer,
  ...
  switch (t) {
    ...
    case LUA_TNIL:
      ...
      break;
    default:
      snprintf(buffer, size, "[%s:%p]", lua_typename(L, t),
...
static void mark_table(lua_State *L, lua_State *dL, const void *parent,
                       const char *desc) {
...
static void mark_userdata(lua_State *L, lua_State *dL, const void *parent,
                          const char *desc) {
  const void *t = readobject(L, dL, parent, desc);
  if (t == NULL) return;
  if (lua_getmetatable(L, -1)) {
...
static void mark_function(lua_State *L, lua_State *dL, const void *parent,
                          const char *desc) {
  const void *t = readobject(L, dL, parent, desc);
  if (t == NULL) return;
...
static void mark_thread(lua_State *L, lua_State *dL, const void *parent,
                        const char *desc) {
  const void *t = readobject(L, dL, parent, desc);
  if (t == NULL) return;
  int level = 0;
  ...
  while (lua_getstack(cL, level, &ar)) {
    ...
    for (j = 1; j > -1; j -= 2) {
      for (i = j;; i += j) {
        ...
        if (name == NULL) break;
        snprintf(tmp, sizeof(tmp), "%s : %s:%d", name, ar.short_src,
                 ar.currentline);
...
static void gen_table_desc(lua_State *dL, luaL_Buffer *b, const void *parent,
                           const char *desc) {
  char tmp[32];
