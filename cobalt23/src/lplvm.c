...
const char *match(lua_State *L, const char *o, const char *s, const char *e,
                  Instruction *op, Capture *capture, int ptop, short *labelf,
  ...
  byte insidepred =
      OUTPRED; /* labeled failure: label environment is off inside predicates */
  stack->p = &giveup;
  stack->s = s;
  stack->caplevel = 0;
  stack->labenv = insidepred;
  stack++;    /* labeled failure */
  *sfail = s; /* labeled failure */
  ...
  for (;;) {
    ...
    printf("s: |%s| stck:%d, dyncaps:%d, caps:%d  ", s,
           (int)(stack - getstackbase(L, ptop)), ndyncap, captop);
    printinst(op, p);
#endif
    ...
    switch ((Opcode)p->i.code) {
      ...
      fail : {                     /* pattern failed: try to backtrack */
        ...
        if (ndyncap > 0) /* is there matchtime captures? */
          ndyncap -= removedyncap(L, capture, stack->caplevel, captop);
        captop = stack->caplevel;
        assert((insidepred == INPRED && stack->labenv == OUTPRED) ||
