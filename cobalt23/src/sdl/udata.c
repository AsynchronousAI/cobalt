 ...
 * associates the userdata with the passed id and pushes the userdata on the
 * stack.
 *
 * The userdata can be subsquently pushed on the Lua stack with udata_push(L,
 * id). (This is useful to bind C/C++ objects to Lua userdata).
 *
 * If id=0, the pointer to the memory area allocated by Lua is used as
 * identifier (this function returnes it).
 */
{
  udata_t *udata;
...
/* unreference udata so that it will be garbage collected */
{
  ...
  udata_t *udata = udata_search(id);
  if (!udata) return luaL_error(L, "invalid object identifier %p", id);
 ...
 */
{
  ...
  if (udata->ref != LUA_NOREF) luaL_unref(L, LUA_REGISTRYINDEX, udata->ref);
  udata_remove(udata);
  Free(L, udata);
...
int udata_push(lua_State *L, uint64_t id) {
  udata_t *udata = udata_search(id);
  if (!udata) return luaL_error(L, "invalid object identifier %p", id);
  if (udata->ref == LUA_NOREF) return luaL_error(L, "unreferenced object");
...
int udata_scan(lua_State *L, const char *mt, void *info,
               int (*func)(lua_State *L, const void *mem, const char *mt,
                           const void *info))
/* scans the udata database, and calls the func callback for every 'mt' object
 * found (the object may be deleted in the callback). func must return 0 to
 * continue the scan, !=0 to interrupt it. returns 1 if interrupted, 0 otherwise
 */
{
  int stop = 0;
  uint64_t id = 0;
...
static int is_subclass(lua_State *L, int arg, int mt_index) {
  int ok;
  if (luaL_getmetafield(L, arg, "__index") == LUA_TNIL) return 0;
 ...
 */
{
  void *mem;
  int ok;
  if ((mem = luaL_testudata(L, arg, mt))) return mem;

  if ((mem = lua_touserdata(L, arg)) == NULL) return NULL;
...
int udata_define(lua_State *L, const char *mt, const luaL_Reg *methods,
  ...
  if (metamethods) /* add metamethods */
 ...
 *
 * This way, if one accesses a field/method of a 'sub' object which is not
 * defined in the 'sub' metatable, Lua searches for it in the 'super' metatable
 * (i.e., the 'sub' type inherits from 'super').
 */
{
