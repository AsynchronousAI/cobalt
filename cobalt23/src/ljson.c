...
#define CJSON_VERSION "0.0.0"
#endif

#ifdef _MSC_VER
#define CJSON_EXPORT __declspec(dllexport)
#else
#define CJSON_EXPORT LUALIB_API
#endif
...
static const char *json_token_type_name[] = {
    "T_OBJ_BEGIN", "T_OBJ_END",    "T_ARR_BEGIN", "T_ARR_END", "T_STRING",
...
static json_config_t *json_arg_init(lua_State *l, int args) {
  ...
  while (lua_gettop(l) < args) lua_pushnil(l);
...
static int json_integer_option(lua_State *l, int optindex, int *setting,
  ...
  if (!lua_isnil(l, optindex)) {
    ...
    snprintf(errmsg, sizeof(errmsg), "expected integer between %d and %d", min,
             max);
...
void json_verify_invalid_number_setting(lua_State *l, int *setting) {
  if (*setting == 1) {
    *setting = 0;
    luaL_error(l,
               "Infinity, NaN, and/or hexadecimal numbers are not supported.");
  }
}
#else
...
static void json_create_config(lua_State *l) {
  ...
  for (i = 0; i < 256; i++) cfg->escape2char[i] = 0; /* String error */
  cfg->escape2char['"'] = '"';
  cfg->escape2char['\\'] = '\\';
  cfg->escape2char['/'] = '/';
  cfg->escape2char['b'] = '\b';
...
static void json_encode_exception(lua_State *l, json_config_t *cfg,
                                  strbuf_t *json, int lindex,
                                  const char *reason) {
  if (!cfg->encode_keep_buffer) strbuf_free(json);
  luaL_error(l, "Cannot serialise %s: %s", lua_typename(l, lua_type(l, lindex)),
             reason);
}
...
static void json_append_array(lua_State *l, json_config_t *cfg,
                              int current_depth, strbuf_t *json,
                              int array_length) {
  int comma, i;
...
static void json_append_number(lua_State *l, json_config_t *cfg, strbuf_t *json,
                               int lindex) {
...
static void json_throw_parse_error(lua_State *l, json_parse_t *json,
  ...
  luaL_error(l, "Expected %s but found %s at character %d", exp, found,
             token->index + 1);
}
