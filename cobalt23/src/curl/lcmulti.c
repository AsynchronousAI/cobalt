...
void lcurl__multi_assign_lua(lua_State *L, lcurl_multi_t *p, lua_State *value,
                             int assign_easy) {
...
int lcurl_multi_create(lua_State *L, int error_mode) {
  ...
  if (lua_type(L, 1) == LUA_TTABLE) {
    int ret = lcurl_utils_apply_options(
        L, 1, 2, 1, p->err_mode, LCURL_ERROR_MULTI, CURLM_UNKNOWN_OPTION);
    if (ret) return ret;
...
static int lcurl_multi_add_handle(lua_State *L) {
  ...
  curL = p->L;
  lcurl__multi_assign_lua(L, p, L, 1);
...
CURLMcode lcurl__multi_remove_handle(lua_State *L, lcurl_multi_t *p,
                                     lcurl_easy_t *e) {
  CURLMcode code;
  lua_State *curL;
...
static int lcurl_multi_perform(lua_State *L) {
  ...
  curL = p->L;
  lcurl__multi_assign_lua(L, p, L, 1);
  while ((code = curl_multi_perform(p->curl, &running_handles)) ==
...
static int lcurl_multi_info_read(lua_State *L) {
  ...
  if (msg->msg == CURLMSG_DONE) {
    ...
    return 1 + lcurl_fail_ex(L, LCURL_ERROR_RETURN, LCURL_ERROR_EASY,
                             msg->data.result);
  }
...
static int lcurl_multi_socket_action(lua_State *L) {
  ...
  if (s == CURL_SOCKET_TIMEOUT)
    mask = lutil_optint64(L, 3, 0);
  else
    mask = lutil_checkint64(L, 3);
...
#define LCURL_LNG_OPT(N, S)                      \
  static int lcurl_multi_set_##N(lua_State *L) { \
    return lcurl_opt_set_long_(L, CURLMOPT_##N); \
  }

#define LCURL_STR_ARR_OPT(N, S)                          \
  static int lcurl_multi_set_##N(lua_State *L) {         \
...
static int lcurl_multi_set_callback(lua_State *L, lcurl_multi_t *p,
...
static int lcurl_multi_set_TIMERFUNCTION(lua_State *L) {
  lcurl_multi_t *p = lcurl_getmulti(L);
  return lcurl_multi_set_callback(L, p, &p->tm, CURLMOPT_TIMERFUNCTION,
                                  CURLMOPT_TIMERDATA, "timer",
                                  lcurl_multi_timer_callback);
}
...
static int lcurl_multi_socket_callback(CURL *easy, curl_socket_t s, int what,
                                       void *arg, void *socketp) {
  lcurl_multi_t *p = arg;
  lua_State *L = p->L;
  lcurl_easy_t *e;
  int n, top;
...
static int lcurl_multi_set_SOCKETFUNCTION(lua_State *L) {
  lcurl_multi_t *p = lcurl_getmulti(L);
  return lcurl_multi_set_callback(L, p, &p->sc, CURLMOPT_SOCKETFUNCTION,
                                  CURLMOPT_SOCKETDATA, "socket",
                                  lcurl_multi_socket_callback);
}
...
static int lcurl_multi_setopt(lua_State *L) {
  ...
  if (lua_type(L, 2) == LUA_TTABLE) {
    int ret = lcurl_utils_apply_options(
        L, 2, 1, 0, p->err_mode, LCURL_ERROR_MULTI, CURLM_UNKNOWN_OPTION);
    if (ret) return ret;
    lua_settop(L, 1);
    return 1;
  }
