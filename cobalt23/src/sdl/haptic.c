...
#define GetInt(field, name)              \
  do {                                   \
    lua_getfield(L, arg, name);          \
    P.field = luaL_optinteger(L, -1, 0); \
    lua_pop(L, 1);                       \
  } while (0)

#define GetInt3(field, name)                  \
  do {                                        \
    lua_getfield(L, -1, name);                \
    if (lua_istable(L, -1)) {                 \
      lua_rawgeti(L, -1, 1);                  \
      P.field[0] = luaL_optinteger(L, -1, 0); \
      lua_pop(L, 1);                          \
      lua_rawgeti(L, -1, 2);                  \
      P.field[1] = luaL_optinteger(L, -1, 0); \
      lua_pop(L, 1);                          \
      lua_rawgeti(L, -1, 3);                  \
      P.field[2] = luaL_optinteger(L, -1, 0); \
      lua_pop(L, 1);                          \
...
#define GetDir()                                     \
  do {                                               \
    lua_getfield(L, arg, "dirtype");                 \
    P.direction.type = opthapticdirection(L, -1, 0); \
    lua_pop(L, 1);                                   \
    GetInt3(direction.dir, "dir");                   \
  } while (0)

static int checkhaptic_constant(lua_State *L, int arg,
...
static int checkhaptic_periodic(lua_State *L, int arg,
                                SDL_HapticEffect *effect) {
...
static int checkhaptic_leftright(lua_State *L, int arg,
                                 SDL_HapticEffect *effect) {
...
static int checkhaptic_condition(lua_State *L, int arg,
                                 SDL_HapticEffect *effect) {
...
#define P effect->custom
  ...
  if (count != (P.samples * P.channels)) {
    Free(L, P.data);
    return luaL_error(L, "data must contain samples*channels samples");
  }
  return 0;
#undef P
}
...
static void freehaptic_effect(lua_State *L, SDL_HapticEffect *effect) {
  if (effect->type == SDL_HAPTIC_CUSTOM) Free(L, effect->custom.data);
}
...
static int checkhaptic_effect(lua_State *L, int arg, SDL_HapticEffect *effect) {
  ...
  switch (effect->type) {
    case SDL_HAPTIC_CONSTANT:
      return checkhaptic_constant(L, arg, effect);
    case SDL_HAPTIC_SINE:
    case SDL_HAPTIC_TRIANGLE:
    case SDL_HAPTIC_SAWTOOTHUP:
    case SDL_HAPTIC_SAWTOOTHDOWN:
      return checkhaptic_periodic(L, arg, effect);
    case SDL_HAPTIC_LEFTRIGHT:
      return checkhaptic_leftright(L, arg, effect);
    case SDL_HAPTIC_RAMP:
      return checkhaptic_ramp(L, arg, effect);
    case SDL_HAPTIC_SPRING:
    case SDL_HAPTIC_DAMPER:
    case SDL_HAPTIC_INERTIA:
    case SDL_HAPTIC_FRICTION:
      return checkhaptic_condition(L, arg, effect);
    case SDL_HAPTIC_CUSTOM:
      return checkhaptic_custom(L, arg, effect);
    ...
    default:
      return luaL_argerror(L, arg, "invalid haptic type");
  }
  return 0;
}
...
#define Set(code, name)                  \
  do {                                   \
    lua_pushboolean(L, supported &code); \
...
static int RunEffect(lua_State *L) {
  ...
  int id = luaL_checkinteger(L, 2);
  uint32_t iterations =
      luaL_optinteger(L, 3, 1);  // see also sdl.HAPTIC_INFINITY
