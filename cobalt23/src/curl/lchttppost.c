...
static lcurl_hpost_stream_t *lcurl_hpost_stream_add(lua_State *L,
                                                    lcurl_hpost_t *p) {
...
static int lcurl_hpost_add_content(lua_State *L) {
  ...
  size_t cont_len;
  const char *cont = luaL_checklstring(L, 3, &cont_len);
  ...
  if (list) {
    forms[i].option = CURLFORM_CONTENTHEADER;
    forms[i++].value = (char *)list;
  }
  forms[i].option = CURLFORM_END;

  code = curl_formadd(&p->post, &p->last, CURLFORM_PTRNAME, name,
                      CURLFORM_NAMELENGTH, (long)name_len, CURLFORM_PTRCONTENTS,
                      cont, LCURL_FORM_CONTENTLEN, (LCURL_LEN_TYPE)cont_len,
...
static int lcurl_hpost_add_buffer(lua_State *L) {
  ...
  size_t cont_len;
  const char *cont = luaL_checklstring(L, 4, &cont_len);
  const char *type = lua_tostring(L, 5);
  struct curl_slist *list =
      lcurl_util_to_slist(L, ((!type) && (lua_isnone(L, 6))) ? 5 : 6);
  struct curl_forms forms[3];
  CURLFORMcode code;
  ...
  code = curl_formadd(&p->post, &p->last, CURLFORM_PTRNAME, name,
                      CURLFORM_NAMELENGTH, (long)name_len, CURLFORM_BUFFER,
                      buff, CURLFORM_BUFFERPTR, cont, CURLFORM_BUFFERLENGTH,
                      cont_len, CURLFORM_ARRAY, forms, CURLFORM_END);
...
static int lcurl_hpost_add_file(lua_State *L) {
  ...
  // add_file("Picture", "c:\\image.jpg", "image/jpeg", "avatar.jpeg",
  // {"XDescript: my image"}) add_file("Picture", "c:\\image.jpg", nil,
  // "avatar.jpeg", {"XDescript: my image"})
  ...
  code = curl_formadd(&p->post, &p->last, CURLFORM_PTRNAME, name,
                      CURLFORM_NAMELENGTH, (long)name_len, CURLFORM_FILE, path,
                      CURLFORM_ARRAY, forms, CURLFORM_END);
...
static int lcurl_hpost_add_stream(lua_State *L) {
  ...
  n = 0;
  if (fname) {
    forms[n].option = CURLFORM_FILENAME;
    forms[n++].value = fname;
  }
  if (type) {
    forms[n].option = CURLFORM_CONTENTTYPE;
    forms[n++].value = type;
  }
  if (list) {
    forms[n].option = CURLFORM_CONTENTHEADER;
    forms[n++].value = (char *)list;
  }
  ...
  code = curl_formadd(&p->post, &p->last, CURLFORM_PTRNAME, name,
                      CURLFORM_NAMELENGTH, (long)name_len, CURLFORM_STREAM,
                      stream, LCURL_FORM_CONTENTLEN, (LCURL_LEN_TYPE)len,
                      CURLFORM_ARRAY, forms, CURLFORM_END);
...
static int lcurl_hpost_add_files(lua_State *L) {
  ...
  if (lua_type(L, -1) != LUA_TTABLE) {
    ...
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_FORM,
  ...
  for (i = 1; i <= arr_count; ++i) {
    ...
    if (n == 1)
      ...
      opt_count += 2;  // name and type
    else if (n == 3)
      opt_count += 3;  // name, type and filename
    else {
      //! @fixme use library specific error codes
      return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_FORM,
                           CURL_FORMADD_ILLEGAL_ARRAY);
  ...
  for (i = 1; i <= arr_count; ++i) {
    ...
    if (lua_type(L, -1) == LUA_TSTRING) {
      forms[opt_count].option = CURLFORM_FILE;
      forms[opt_count++].value = luaL_checkstring(L, -1);
    } else {
      n = lua_rawlen(L, -1);
      lua_rawgeti(L, -1, 1);
      forms[opt_count].option = CURLFORM_FILE;
      ...
      if (n > 1) {
        lua_rawgeti(L, -1, 2);
        forms[opt_count].option = CURLFORM_CONTENTTYPE;
        forms[opt_count++].value = luaL_checkstring(L, -1);
        lua_pop(L, 1);
      }
      if (n > 2) {
        lua_rawgeti(L, -1, 3);
        forms[opt_count].option = CURLFORM_FILENAME;
        forms[opt_count++].value = luaL_checkstring(L, -1);
        lua_pop(L, 1);
      }
    }
  ...
  code = curl_formadd(&p->post, &p->last, CURLFORM_PTRNAME, name,
                      CURLFORM_NAMELENGTH, (long)name_len, CURLFORM_ARRAY,
                      forms, CURLFORM_END);
...
static size_t lcurl_hpost_getter_by_buffer(void *arg, const char *buf,
                                           size_t len) {
  luaL_Buffer *b = arg;
...
static size_t call_writer(lua_State *L, int fn, int ctx, const char *buf,
                          size_t len) {
  int top = lua_gettop(L);
...
static size_t lcurl_hpost_getter_by_callback1(void *arg, const char *buf,
                                              size_t len) {
  lua_State *L = arg;
...
static size_t lcurl_hpost_getter_by_callback2(void *arg, const char *buf,
                                              size_t len) {
  lua_State *L = arg;
...
static int lcurl_hpost_get(lua_State *L) {
  ...
  if (lua_isfunction(L, 2)) {
    ...
    luaL_argcheck(L, lua_isfunction(L, -1), 2,
                  "write method not found in object");
    assert(3 == lua_gettop(L));
    lua_insert(L, -2);
    top = 3;
    code = curl_formget(p->post, L, lcurl_hpost_getter_by_callback2);
