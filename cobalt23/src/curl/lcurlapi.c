...
int lcurl_url_create(lua_State *L, int error_mode) {
  ...
  p->url = curl_url();
  if (!p->url)
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_URL, CURLUE_OUT_OF_MEMORY);
...
#define ENTRY_PART(N, S, E)                    \
  static int lcurl_url_set_##N(lua_State *L) { \
...
#define ENTRY_PART(N, S, E)                    \
  static int lcurl_url_get_##N(lua_State *L) { \
...
#if LCURL_CURL_VER_GE(7, 62, 0)
  if (!lutil_createmetap(L, LCURL_URL, lcurl_url_methods, nup)) lua_pop(L, nup);
  lua_pop(L, 1);
