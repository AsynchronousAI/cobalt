...
static int freejoystick(lua_State *L, ud_t *ud) {
  ...
  if (!borrowed) SDL_JoystickClose(joystick);
  return 0;
}
...
static int Rumble(lua_State *L) {
  ...
  if (SDL_JoystickRumble(joystick, low_frequency_rumble, high_frequency_rumble,
                         duration_ms) != 0)
    GetError(L);
  return 0;
}
...
static int RumbleTriggers(lua_State *L) {
  ...
  uint32_t duration_ms = checkmilliseconds(L, 4);
  if (SDL_JoystickRumbleTriggers(joystick, left_rumble, right_rumble,
                                 duration_ms) != 0)
    GetError(L);
  return 0;
}
...
static int NumJoysticks(
    lua_State *L) /* joystick_index = 1, ..., NumJoysticks */
{
