...
static int doargs(int argc, char* argv[]) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    } else if (IS("-i")) /* preprocess file */
    {
      ...
      fatal(
          "inputting preprocessor definitions not supported yet. use cobaltpre "
          "to input preprocessor definitions.");
...
static int pmain(lua_State* L) {
  ...
  for (i = 0; i < argc; i++) {
    ...
    if (preprocess) {
      ...
      /*
      - cobalt -e "cobalt -e 'import("preprocess")("<INPUT>", "file", true,
      "<PROCESS>")'"
      */
      char command[256];
...
#define ISK (isk ? "k" : "")
#define PRINT_CODE_LINE(line) printf(" @%d", line)
...
static void PrintDebug(const Proto* f) {
  ...
  for (i = 0; i < n; i++) {
    printf("\t%d\t%s\t%d\t%d\t@%d\t@%d\n", i, getstr(f->locvars[i].varname),
           ...
           luaG_getfuncline(f, f->locvars[i].startpc),
           luaG_getfuncline(f, f->locvars[i].endpc));
  }
  n = f->sizeupvalues;
