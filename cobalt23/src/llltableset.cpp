...
void TableSet::PerformGet() {
  ...
  auto nilobj = cs_.InjectPointer(ttvalue, const_cast<TValue*>(luaO_nilobject));
  cs_.B_.CreateBr(finishset_);
  oldvals_.push_back({nilobj, getnil_});
}

void TableSet::CallGCBarrier() {
  auto checktableblack = cs_.CreateSubBlock("checktableblack", callgcbarrier_);
  auto checkvaluewhite = cs_.CreateSubBlock("checkvaluewhite", checktableblack);
  auto callbarrierback = cs_.CreateSubBlock("callbarrierback", checkvaluewhite);
  ...
  auto tablemarked =
      cs_.LoadField(tablevalue_, tlu_byte, markedoffset, "tablemarked");
  ...
  auto valuemarked =
      cs_.LoadField(gcvalue, tlu_byte, markedoffset, "valuemarked");
...
void TableSet::FinishSet() {
  ...
  auto args = {cs_.values_.state, table_.GetTValue(), key_.GetTValue(),
               value_.GetTValue(), oldvalphi};
