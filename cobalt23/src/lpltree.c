...
static int lp_utfr(lua_State *L) {
  ...
  if (to <= 0x7f) {              /* ascii range? */
    ...
    unsigned int f;
    for (f = (int)from; f <= to; f++) setchar(treebuffer(tree), f);
...
static int lp_match(lua_State *L) {
  ...
  if (r == NULL) {   /* labeled failure begin */
    ...
    lua_pushinteger(
        L, sfail - (s + i) + 1); /* subject position related to the error */
    return 3;
  } /* labeled failure end */
...
LUALIB_API int luaopen_lpeg(lua_State *L) { /* labeled failure */
