...
static void* luaM_growaux_(lua_State* L, void* block, int* size,
  ...
  int newsize;
  if (*size >= limit / 2) {
    if (*size >= limit) luaG_runerror(L, errormsg);
    newsize = limit;
  } else {
    newsize = (*size) * 2;
...
static UpVal* luaF_findupval(lua_State* L, StkId level) {
  ...
  while (*pp != NULL && (p = ngcotouv(*pp))->v >= level) {
    if (p->v == level) {
      if (isdead(g, obj2gco(p))) changewhite(obj2gco(p));
      return p;
    }
    pp = &p->next;
  }
...
static void removeentry(Node* n) {
  if (iscollectable(gkey(n))) setttype(gkey(n), (8 + 3));
}
static void reallymarkobject(global_State* g, GCObject* o) {
  ...
  switch (o->gch.tt) {
    ...
    case (8 + 2): {
      UpVal* uv = gco2uv(o);
      markvalue(g, uv->v);
      if (uv->v == &uv->u.value) gray2black(o);
      return;
    }
    case 6: {
...
static l_mem propagatemark(global_State* g) {
  ...
  switch (o->gch.tt) {
    case 5: {
      Table* h = gco2h(o);
      g->gray = h->gclist;
      if (traversetable(g, h)) black2gray(o);
...
static void cleartable(GCObject* l) {
  while (l) {
    ...
    if (testbit(h->marked, 4)) {
      while (i--) {
        TValue* o = &h->array[i];
        if (iscleared(o, 0)) setnilvalue(o);
      }
    }
    i = sizenode(h);
    while (i--) {
...
static GCObject** sweeplist(lua_State* L, GCObject** p, lu_mem count) {
  ...
  while ((curr = *p) != NULL && count-- > 0) {
    if (curr->gch.tt == 8) sweepwholelist(L, &gco2th(curr)->openupval);
...
static void luaC_freeall(lua_State* L) {
  ...
  for (i = 0; i < g->strt.size; i++) sweepwholelist(L, &g->strt.hash[i]);
}
...
static l_mem singlestep(lua_State* L) {
  ...
  switch (g->gcstate) {
    ...
    case 2: {
      ...
      if (g->sweepstrgc >= g->strt.size) g->gcstate = 3;
    ...
    case 4: {
      if (g->tmudata) {
        GCTM(L);
        if (g->estimate > 100) g->estimate -= 100;
        return 100;
      } else {
        g->gcstate = 0;
        g->gcdept = 0;
...
static void read_numeral(LexState* ls, SemInfo* seminfo) {
  ...
  if (check_next(ls, "Ee")) check_next(ls, "+-");
  ...
  for (;;) {
    switch (ls->current) {
      ...
      case '/': {
        ...
        // short comment
        if (ls->current == '/') {
          while (!currIsNewline(ls) && ls->current != (-1)) next(ls);
          break;
          // long comment
        } else if (ls->current == '*') {
          next(ls);
...
static int patchtestreg(FuncState* fs, int node, int reg) {
  ...
  if (GET_OPCODE(*i) != OP_TESTSET) return 0;
...
static void luaK_concat(FuncState* fs, int* l1, int l2) {
  if (l2 == (-1))
    ...
    *l1 = l2;
    ...
    int list = *l1;
    int next;
    while ((next = getjump(fs, list)) != (-1)) list = next;
    fixjump(fs, list, l2);
  }
}
...
static const struct {
  lu_byte left;
  lu_byte right;
} priority[] = {{10, 10}, {10, 10}, {11, 11}, {11, 11}, {14, 13},
                {11, 11}, {6, 6},   {4, 4},   {5, 5},   {7, 7},
                {7, 7},   {9, 8},   {3, 3},   {3, 3},   {3, 3},
                {3, 3},   {3, 3},   {3, 3},   {2, 2},   {1, 1}};
...
static int io_close(lua_State* L) {
  if (lua_isnone(L, 1)) lua_rawgeti(L, (-10001), 2);
  tofile(L);
  return aux_close(L);
}
...
static const char* start_capture(MatchState* ms, const char* s, const char* p,
  ...
  ms->level = level + 1;
  if ((res = match(ms, s, p)) == NULL) ms->level--;
  return res;
}
static const char* end_capture(MatchState* ms, const char* s, const char* p) {
  ...
  if ((res = match(ms, s, p)) == NULL) ms->capture[l].len = (-1);
  return res;
}
...
static const luaL_Reg strlib[] = {
    ...
    {"rep", str_rep},     {"replace", str_replace},
