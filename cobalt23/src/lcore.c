...
static int allocate_hex_memory_address(lua_State *L) {
  ...
  if (memory_map == NULL) {
#if defined __unix__ || LUA_USE_POSIX || __APPLE__
    memory_map = mmap(NULL, MAX_MEMORY, PROT_READ | PROT_WRITE,
                      MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
#elif defined _WIN32 || defined _WIN64 || defined __CYGWIN__ || \
    defined __MINGW32__ || defined LUA_USE_WINDOWS || defined LUA_USE_MINGW
    memory_map = VirtualAlloc(NULL, MAX_MEMORY, MEM_COMMIT | MEM_RESERVE,
                              PAGE_READWRITE);
#else
...
#if defined __unix__ || LUA_USE_POSIX || __APPLE__
  void *address = mmap(memory_map, size, PROT_READ | PROT_WRITE,
                       MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
#else
  void* address =
      VirtualAlloc(memory_map, size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
#endif
...
static int dumpmacros(lua_State *L) {
  ...
  if (lua_isnil(L, -1)) {
    ...
    // 3- Make a new table and for every item in the old table add another table
    // in the newtable with {item:split(" ")[1] = item:split(" ")[2]}
...
static int error(lua_State *L) {
  // It has 2 optional arguments, error message, and exit code. It uses perror
...
char cobalt_errors[][256] = {
    ...
    "Type error",
    //// EXPECTIONS
    "Expected %s, got %s for argument %d", "Expected %s, got %s",
    "Expected %s, got %s for argument %d of %s",
    "Expected '%s' got '%s' in variable '%s'", "%s is not a valid type",

    // MEMORY
    "Failed to allocate memory", "Failed to set memory permissions",
    "Failed to free memory", "Failed to resize memory",

    // OTHER
    "Undisclosed error", "error1", "error2", "error3", "error4", "error5", ""};
...
static const struct luaL_Reg lcore_error_lib[] = {{"cerror", error},
                                                  ...
                                                  {"errorlen", errorlen},
                                                  {NULL, NULL}};
static const struct luaL_Reg lcore_memory_lib[] = {
...
#define GNUC_PREREQ(M, m) \
  (__GNUC__ && ((__GNUC__ > M) || (__GNUC__ == M && __GNUC_MINOR__ >= m)))
...
#define UCLIBC_PREREQ(M, m, p)                                                 \
  (__UCLIBC__ &&                                                               \
   (__UCLIBC_MAJOR__ > M || (__UCLIBC_MAJOR__ == M && __UCLIBC_MINOR__ > m) || \
...
#define FREEBSD_PREREQ(M, m) \
  (__FreeBSD_version && __FreeBSD_version >= ((M)*100000) + ((m)*1000))
...
#define IPHONE_PREREQ(M, m) \
  (IPHONE_2VER((M), (m)) <= __IPHONE_OS_VERSION_MIN_REQUIRED)
#else
...
#define MACOS_PREREQ_10_10(M, m, p)          \
  (((M) > 10 || ((M) == 10 && (m) >= 10)) && \
   MACOS_2VER_10_10((M), (m), (p)) <= __MAC_OS_X_VERSION_MIN_REQUIRED)
#define MACOS_PREREQ_10_9(M, m, p) \
  (((M) == 10 && (m) < 10) &&      \
   MACOS_2VER_10_9((M), (m), (p)) <= __MAC_OS_X_VERSION_MIN_REQUIRED)
#if defined __MAC_OS_X_VERSION_MIN_REQUIRED
#define MACOS_PREREQ(M, m, p) \
  (MACOS_PREREQ_10_10((M), (m), (p)) || MACOS_PREREQ_10_9((M), (m), (p)))
#else
...
#define HAVE_C__GENERIC                                        \
  (GNUC_PREREQ(4, 9) || __has_feature(c_generic_selections) || \
...
#define HAVE_STRUCT_IN_PKTINFO_IPI_SPEC_DST \
  (HAVE_DECL_IP_PKTINFO && !__NetBSD__)
#endif
...
#define HAVE_STRUCT_STAT_ST_ATIMESPEC \
  (__APPLE__ || HAVE_DECL_ST_ATIMESPEC || HAVE_DECL_ST_ATIMENSEC)
#endif
...
#ifndef HAVE_ACCEPT4
#define HAVE_ACCEPT4 \
  (HAVE_DECL_SOCK_CLOEXEC && (!__NetBSD__ || NETBSD_PREREQ(8, 0)))
#endif

#ifndef HAVE_ARC4RANDOM
#define HAVE_ARC4RANDOM \
  (__OpenBSD__ || __FreeBSD__ || __NetBSD__ || __MirBSD__ || __APPLE__)
#endif

#ifndef HAVE_ARC4RANDOM_STIR
#define HAVE_ARC4RANDOM_STIR \
  (HAVE_ARC4RANDOM && (!OpenBSD || OpenBSD < 201405) && !FREEBSD_PREREQ(12, 0))
#endif
...
#define HAVE_FDOPENDIR                                               \
  ((!__APPLE__ || MACOS_PREREQ(10, 10, 0) || IPHONE_PREREQ(8, 0)) && \
   (!__NetBSD__ || NETBSD_PREREQ(6, 0)))
#endif
...
#define HAVE__LIBC_ENABLE_SECURE \
  GLIBC_PREREQ(2, 1) /* added to glibc between 2.0.98 and 2.0.99 */
#endif
...
#define HAVE_MKFIFOAT                                                \
  ((!__APPLE__ || MACOS_PREREQ(13, 0, 0) || IPHONE_PREREQ(16, 0)) && \
   (!__NetBSD__ || NETBSD_PREREQ(7, 0)))
#endif
...
#define HAVE_STATIC_ASSERT_ \
  (!GLIBC_PREREQ(0, 0) ||   \
   HAVE__STATIC_ASSERT) /* glibc doesn't check GCC version */
...
#define HAVE_SYS_SIGLIST \
  (!MUSL_MAYBE && !__sun && !_AIX && !__UCLIBC__ && !GLIBC_PREREQ(2, 32))
#endif
...
#define STRERROR_R_CHAR_P                            \
  ((GLIBC_PREREQ(0, 0) || UCLIBC_PREREQ(0, 0, 0)) && \
...
static void *compatL_testudata(lua_State *L, int index, const char *tname) {
  ...
  if (!p || !lua_getmetatable(L, index)) return 0;
...
#else
#define u_static_assert(cond, msg) \
  extern char XPASTE(assert_, __LINE__)[u_inline_assert(cond)]
#endif
...
#if defined __GNUC__ && \
    (defined _AIX || (defined __NetBSD__ && !NETBSD_PREREQ(6, 0)))
...
static u_error_t u_realloc(char **buf, size_t *size, size_t minsiz) {
  ...
  if (*size == (size_t)-1) return ENOMEM;
  ...
  if (!(tmp = realloc(*buf, tmpsiz))) return errno;
...
static u_error_t u_appendc(char **buf, size_t *size, size_t *p, int c) {
  ...
  if (*p < *size) {
    ...
    if ((error = u_growby(buf, size, (*p - *size) + 1))) return error;
...
static u_error_t u_appends(char **buf, size_t *bufsiz, size_t *p,
   ...
   */
  if (*p > *bufsiz) return EINVAL;

  r = *bufsiz - *p;
  if (r < srclen && (error = u_growby(buf, bufsiz, srclen - r))) return error;
...
static u_error_t u_reallocarray(void **arr, size_t *arrsiz, size_t count,
                                size_t size) {
  void *tmp;
  size_t tmpsiz;

  if (size && count > (size_t)-1 / size) return ENOMEM;

  tmpsiz = count * size;

  if (tmpsiz <= *arrsiz) return 0;

  if (tmpsiz == (size_t)-1) return ENOMEM;
  ...
  if (!(tmp = realloc(*arr, tmpsiz))) return (tmpsiz) ? errno : 0;
  U_WARN_POP
...
MAYBEUSED static void u_in6_prefixlen2mask(struct in6_addr *mask,
                                           unsigned prefixlen) {
  ...
  if (bits) mask->s6_addr[octets] = (0xff00 >> bits) & 0xff;
...
static int u_in6_mask2prefixlen(const struct in6_addr *mask) {
  ...
  for (i = 0; i < 16; i++) {
    for (j = 0; j < 8; j++) {
      if (!((0x80 >> j) & mask->s6_addr[i])) return 8 * i + j;
    }
  }
...
#define IN6_IS_SCOPE_LINKLOCAL(in6) \
  (IN6_IS_ADDR_LINKLOCAL(in6) || IN6_IS_ADDR_MC_LINKLOCAL(in6))
#endif
...
#define IN6_IS_ADDR_MC_INTFACELOCAL(in6) \
  (IN6_IS_ADDR_MULTICAST(in6) ||         \
   IPV6_ADDR_MC_SCOPE(in6) == IPV6_ADDR_SCOPE_INTFACELOCAL)
#endif
...
static int u_f2ms(const double f) {
  ...
  switch (fpclassify(f)) {
    case FP_NORMAL:
      ...
      u_static_assert(u_ispower2((unsigned)INT_MAX + 1),
                      "INT_MAX + 1 not a power of 2");

      if (ms >= (unsigned)INT_MAX + 1) return INT_MAX;
...
static struct timespec *u_f2ts(struct timespec *ts, const double f) {
  ...
  switch (fpclassify(f)) {
    case FP_NORMAL:
      ...
      u_static_assert(FLT_RADIX == 2, "FLT_RADIX value unsupported");
      u_static_assert(u_ispower2((unsigned long)LONG_MAX + 1),
                      "LONG_MAX + 1 not a power of 2");
...
MAYBEUSED static void ts_timeradd(struct timespec *r, struct timespec a,
                                  struct timespec b) {
...
MAYBEUSED static void ts_timersub(struct timespec *r, struct timespec a,
                                  struct timespec b) {
  r->tv_sec = a.tv_sec - b.tv_sec;
  r->tv_nsec = a.tv_nsec - b.tv_nsec;
...
static u_error_t u_sigtimedwait(int *_signo, const sigset_t *set,
                                siginfo_t *_info,
  ...
  *_signo = -1;
  ...
  if (-1 == (signo = sigtimedwait(set, &info, timeout))) return errno;
  ...
  *_signo = -1;
  ...
  do {
    ...
    for (signo = 1; signo < NSIG; signo++) {
      if (!sigismember(set, signo) || !sigismember(&pending, signo)) continue;
  ...
  *_signo = -1;
  ...
  do {
    ...
    for (signo = 1; signo < NSIG; signo++) {
      ...
      if ((error = sigwait(set, &signo))) return error;
...
static u_error_t u_strcpy(char *dst, const char *src, size_t lim) {
  if (u_strlcpy(dst, src, lim) >= lim) return EOVERFLOW;
...
static u_error_t u_strerror_r(int error, char *dst, size_t lim) {
  ...
  if (!(src = strerror_r(error, dst, lim))) return EINVAL;
...
static u_error_t u_setflag(int fd, u_flags_t flag, int enable) {
  ...
  if (flag & U_CLOEXEC) {
    if (-1 == (flags = fcntl(fd, F_GETFD))) return errno;
    ...
    if (0 != fcntl(fd, F_SETFD, flags)) return errno;
  } else {
    if (-1 == (flags = fcntl(fd, F_GETFL))) return errno;
    ...
    if (0 != fcntl(fd, F_SETFL, flags)) return errno;
  }
...
static u_error_t u_getflags(int fd, u_flags_t *flags) {
  ...
  if (-1 == (_flags = fcntl(fd, F_GETFL))) return errno;

  *flags = _flags;

  /* F_GETFL isn't defined to return O_CLOEXEC */
  if (!(*flags & U_CLOEXEC)) {
    if (-1 == (_flags = fcntl(fd, F_GETFD))) return errno;

    if (_flags & FD_CLOEXEC) *flags |= U_CLOEXEC;
  }
...
static u_error_t u_getaccmode(int fd, u_flags_t *oflags, u_flags_t flags) {
  ...
  if (O_ACCMODE & (*oflags = flags)) return 0;
...
static u_error_t u_fixflags(int fd, u_flags_t flags) {
  ...
  if (flags & U_FIXFLAGS) {
    if ((error = u_getflags(fd, &_flags))) return error;

    if ((flags & U_CLOEXEC) && !(_flags & U_CLOEXEC)) {
      if ((error = u_setflag(fd, U_CLOEXEC, 1))) return error;
    }

    if ((flags & O_NONBLOCK) && !(_flags & O_NONBLOCK)) {
      if ((error = u_setflag(fd, O_NONBLOCK, 1))) return error;
    }
  }
...
static u_error_t u_open(int *fd, const char *path, u_flags_t flags,
                        mode_t mode) {
  int error;

  if (-1 == (*fd = open(path, (U_SYSFLAGS & flags), mode))) goto syerr;
  ...
  if ((error = u_fixflags(*fd, flags))) goto error;
...
static u_error_t u_dup2(int fd, int fd2, u_flags_t flags) {
   ...
   */
  if ((error = u_fixflags(fd, U_FLFLAGS & flags))) return error;

  flags &= ~U_FLFLAGS;

#if HAVE_DUP3
  if (-1 == dup3(fd, fd2, U_SYSFLAGS & flags)) return errno;
...
static u_error_t u_dup(int *fd, int ofd, u_flags_t flags) {
  ...
  if (-1 == (*fd = fcntl(ofd, cmd, 0))) goto syerr;
...
MAYBEUSED static u_error_t u_socket(int *fd, int family, int type, int proto,
                                    u_flags_t flags) {
  int error;

#if defined SOCK_CLOEXEC
  if (flags & U_CLOEXEC) type |= SOCK_CLOEXEC;
  ...
  if (flags & O_NONBLOCK) type |= SOCK_NONBLOCK;

  if (type & SOCK_NONBLOCK) flags &= ~O_NONBLOCK;
#endif

  if (-1 == (*fd = socket(family, type, proto))) return errno;
...
static u_error_t u_fdopendir(DIR **dp, int *fd, _Bool nodup MAYBEUSED) {
  ...
  *dp = NULL;
  ...
  if (!(*dp = fdopendir(*fd))) return errno;
  ...
  *dp = NULL;
  ...
  if (nodup) return ENOTSUP;

  if (0 != fstat(*fd, &st)) goto syerr;
  ...
  for (path = local; path < endof(local); path++) {
    if ((*dp = opendir(*path))) break;
  }

  if (!*dp) goto syerr;

  if (-1 == (fd2 = dirfd(*dp))) goto syerr;

  if ((error = u_dup2(*fd, fd2, U_CLOEXEC))) goto error;

  if (-1 == lseek(fd2, 0, SEEK_SET)) goto syerr;
...
#define u_strmode(...) \
  u_strmode_(__VA_ARGS__, &(char[U_MODESTRLEN]){0}, U_MODESTRLEN)
  ...
  while ((ch = *mode++)) {
    switch (ch) {
      ...
      case 'x':
        ...
        break;
      default:
        if (!isalpha((unsigned char)ch)) goto done;
...
static int u_fdopen(FILE **fp, int *fd, const char *mode, u_flags_t flags) {
  ...
  if (!mode) {
    if ((error = u_getaccmode(*fd, &flags, flags))) return error;

    mode = u_strmode(flags, mbuf, sizeof mbuf);
  }

  if ((error = u_fixflags(*fd, flags))) return error;

  if (!(*fp = fdopen(*fd, mode))) return errno;
...
static u_error_t u_getifconf(struct ifconf *ifc, int fd) {
  ...
  do {
    ...
    if (bufsiz < U_IFREQ_MAXSIZE) goto range;

    if ((error = u_realloc(&buf, &bufsiz, MAX(256, bufsiz)))) goto error;

    /* ifc->ifc_len is usually an int; be careful of undefined conversion */
    if (bufsiz > INT_MAX) goto range;
    ...
    if (-1 == ioctl(fd, SIOCGIFCONF, (void *)ifc)) goto syerr;
...
static u_error_t u_getif6_aix(struct u_ifaddrs *ifa, int fd,
                              const struct ifreq *ifr) {
  struct in6_ifreq ifr6 = {0};

  u_static_assert(sizeof ifr6.ifr_name >= sizeof ifr->ifr_name,
                  "sizeof ifr6_name < sizeof ifr_name");
  memcpy(ifr6.ifr_name, ifr->ifr_name,
         MIN(sizeof ifr6.ifr_name, sizeof ifr->ifr_name));
  ...
  if (-1 != ioctl(fd, SIOCGIFNETMASK6, &ifr6)) {
    ...
    ifa->ifa_netmask =
        u_sa_copy(&ifa->ifa_ss[1], (struct sockaddr *)&ifr6.ifr_Addr);
  }

  if (-1 != ioctl(fd, SIOCGIFDSTADDR6, &ifr6)) {
    ifr6.ifr_Addr.sin6_family =
        AF_INET6; /* unable to test if AIX sets family; see above */
    ifa->ifa_dstaddr =
        u_sa_copy(&ifa->ifa_ss[2], (struct sockaddr *)&ifr6.ifr_Addr);
  }
...
static u_error_t u_getif6_kame_in6(struct u_ifaddrs *ifa, int fd,
                                   const struct ifreq *ifr) {
  struct in6_ifreq ifr6 = {0};
...
static u_error_t u_getif6_kame_glif(struct u_ifaddrs *ifa, int fd,
                                    const struct ifreq *ifr) {
  struct if_laddrreq iflr = {0};

  u_static_assert(sizeof iflr.iflr_name >= sizeof ifr->ifr_name,
                  "sizeof iflr_name < sizeof ifr_name");
  memcpy(iflr.iflr_name, ifr->ifr_name,
         MIN(sizeof iflr.iflr_name, sizeof ifr->ifr_name));
  ...
  if (-1 != ioctl(fd, SIOCGLIFADDR, &iflr)) {
    ...
    if (iflr.dstaddr.ss_family == AF_INET6)
      ifa->ifa_dstaddr =
          u_sa_copy(&ifa->ifa_ss[2], (struct sockaddr *)&iflr.dstaddr);
  }
...
static u_error_t u_getif6_sun_glif(struct u_ifaddrs *ifa, int fd,
                                   const struct ifreq *ifr) {
  struct lifreq lifr = {0};

  u_static_assert(sizeof lifr.lifr_name >= sizeof ifr->ifr_name,
                  "sizeof iflr_name < sizeof ifr_name");
  memcpy(lifr.lifr_name, ifr->ifr_name,
         MIN(sizeof lifr.lifr_name, sizeof ifr->ifr_name));
  u_sa_copy(&lifr.lifr_addr, ifa->ifa_addr);

  if (-1 != ioctl(fd, SIOCGLIFNETMASK, &lifr)) {
    ifa->ifa_netmask =
        u_sa_copy(&ifa->ifa_ss[1], (struct sockaddr *)&lifr.lifr_dstaddr);
  }

  if (-1 != ioctl(fd, SIOCGLIFDSTADDR, &lifr)) {
    ifa->ifa_dstaddr =
        u_sa_copy(&ifa->ifa_ss[2], (struct sockaddr *)&lifr.lifr_dstaddr);
  }

  if (-1 != ioctl(fd, SIOCGLIFBRDADDR, &lifr)) {
    ifa->ifa_dstaddr =
        u_sa_copy(&ifa->ifa_ss[2], (struct sockaddr *)&lifr.lifr_broadaddr);
  }
...
static u_error_t u_getifaddrs(struct u_ifaddrs **ifs) {
  ...
  *ifs = NULL;
  ...
  if ((error = u_getifconf(&ifc, fd))) goto error;
  ...
  while (ifr < end) {
    ...
    if (!(ifa = calloc(1, sizeof *ifa))) goto syerr;

    u_static_assert(sizeof ifa->ifa_name >= sizeof ifr->ifr_name,
                    "sizeof ifa_name < sizeof ifr_name");
    memcpy(ifa->ifa_name, ifr->ifr_name,
           MIN(sizeof ifa->ifa_name, sizeof ifr->ifr_name));

    ifa->ifa_addr = u_sa_copy(&ifa->ifa_ss[0], &ifr->ifr_addr);

    if (-1 != ioctl(fd, SIOCGIFFLAGS, ifr)) ifa->ifa_flags = ifr->ifr_flags;

    if (ifa->ifa_addr->sa_family == AF_INET6) {
#if defined u_getif6
      // fprintf(stderr, "u_getif6:%s\n", STRINGIFY(u_getif6));
      if (fd6 == -1 &&
          (error = u_socket(&fd6, AF_INET6, SOCK_DGRAM, PF_UNSPEC, U_CLOEXEC)))
        goto error;

      if ((error = u_getif6(ifa, fd6, ifr))) goto error;
#endif
    } else {
      ...
      else if (-1 != ioctl(fd, SIOCGIFBRDADDR, ifr) &&
               ifr->ifr_addr.sa_family == ifa->ifa_addr->sa_family)
        ifa->ifa_dstaddr = u_sa_copy(&ifa->ifa_ss[2], &ifr->ifr_addr);
    }
...
static int u_getgrgid_r(gid_t gid, struct group *grp, char *buf, size_t bufsiz,
                        struct group **res) {
#if GETGRGID_R_AIX
  if (bufsiz == 0) return ERANGE;

  errno = 0;
  return getgrgid_r(gid, grp, buf, bufsiz, res) ? ((errno != ESRCH) ? errno : 0)
                                                : 0;
...
static int u_getgrnam_r(const char *nam, struct group *grp, char *buf,
                        size_t bufsiz, struct group **res) {
#if GETGRNAM_R_AIX
  if (bufsiz == 0) return ERANGE;

  errno = 0;
  return getgrnam_r(nam, grp, buf, bufsiz, res) ? ((errno != ESRCH) ? errno : 0)
                                                : 0;
...
static int u_getpwuid_r(uid_t uid, struct passwd *pwd, char *buf, size_t bufsiz,
                        struct passwd **res) {
#if GETPWUID_R_AIX
  if (bufsiz == 0) return ERANGE;

  errno = 0;
  return getpwuid_r(uid, pwd, buf, bufsiz, res) ? ((errno != ESRCH) ? errno : 0)
                                                : 0;
...
static int u_getpwnam_r(const char *nam, struct passwd *pwd, char *buf,
                        size_t bufsiz, struct passwd **res) {
#if GETPWNAM_R_AIX
  if (bufsiz == 0) return ERANGE;

  errno = 0;
  return getpwnam_r(nam, pwd, buf, bufsiz, res) ? ((errno != ESRCH) ? errno : 0)
                                                : 0;
...
static u_error_t u_ptsname_r(int fd, char *buf, size_t buflen) {
  ...
  if (0 != ptsname_r(fd, buf, buflen)) return errno;
   ...
   */
  errno = 0;
  if (!(path = ptsname(fd))) return (errno) ? errno : EINVAL;

  if (u_strlcpy(buf, path, buflen) >= buflen) return ERANGE;
...
static u_error_t u_getsockport(int fd, in_port_t *port,
                               int (*getname)(int, struct sockaddr *,
                                              socklen_t *)) {
  union {
  ...
  if (0 != getname(fd, (struct sockaddr *)&addr, &addrlen)) return errno;
...
static ssize_t u_recvfromto(int fd, void *buf, size_t lim, int flags,
  ...
  if ((*error = u_getsockport(fd, &to_port, &getsockname))) return -1;
  ...
  *fromlen = msg.msg_namelen;
  ...
  for (cmsg = CMSG_FIRSTHDR(&msg); cmsg != NULL;
    ...
    if (cmsg->cmsg_level == IPPROTO_IPV6 && cmsg->cmsg_type == IPV6_PKTINFO) {
      ...
      if (*tolen < sizeof *in6) goto inval;
...
static ssize_t u_recvfromto(int fd, const void *src, size_t len, int flags,
...
static ssize_t u_sendtofrom(int fd, const void *buf, size_t len, int flags,
                            const struct sockaddr *to, size_t tolen,
                            const struct sockaddr *from, size_t fromlen,
                            u_error_t *error) {
  struct iovec iov;
  ...
  switch (from->sa_family) {
    ...
    case AF_INET:
      ...
      if (sizeof *in < fromlen) goto inval;
    ...
    case AF_INET6:
      ...
      if (sizeof *in6 < fromlen) goto inval;
  ...
  if (-1 == (n = sendmsg(fd, &msg, flags))) *error = errno;
...
static ssize_t u_sendtofrom(int fd, const void *src, size_t len, int flags,
                            const struct sockaddr *to, size_t tolen,
                            const struct sockaddr *from, size_t fromlen,
                            u_error_t *error) {
  (void)fd;
...
#define UNIXL_RANDOM_INITIALIZER \
  { .fd = -1, }
...
static void arc4_stir(unixL_Random *R, int force) {
  ...
  if (R->count > 0 && R->pid == getpid() && !force) return;
  ...
  if (count < sizeof bytes) {
    if (R->fd == -1 &&
        0 != u_open(&R->fd, "/dev/urandom", O_RDONLY | U_CLOEXEC, 0))
      goto stir;
...
stir:
  ...
  for (n = 0; n < 1024; n++) arc4_getbyte(R);
...
#define U_GETOPT_R_INITIALIZER \
  { NULL, 1, 1, 0, 0, NULL }
#define U_GETOPT_R_INIT(k)                          \
  do {                                              \
...
static const char *getopt_progname(int argc, char *const argv[],
                                   struct u_getopt_r *K) {
  (void)K;
  return (argc > 0 && argv[0]) ? argv[0] : "";
}

static void getopt_err(int argc, char *const argv[], const char *shortopts,
                       struct u_getopt_r *K, const char *fmt, ...) {
  va_list ap;

  if (*shortopts == ':' || !K->opterr) return;
...
#define GETOPT_YIELD(rv) GETOPT_SAVE_AND_DO(return (rv))
...
static int u_getopt_r(int argc, char *const argv[], const char *shortopts,
                      struct u_getopt_r *K) {
  K->optarg = NULL;
  K->optopt = 0;
  ...
  while (K->optind < argc) {
    ...
    for (;;) {
      ...
      if (!(K->optopt = *++K->cp)) {
        ...
        getopt_err(argc, argv, shortopts, K,
 ...
 */
static int unixL_newmetatable(lua_State *L, const char *name,
                              const luaL_Reg *methods,
                              const luaL_Reg *metamethods, int nup) {
  int i, n;

  if (!luaL_newmetatable(L, name)) return 0;

  /* add metamethods */
  for (i = 0; i < nup; i++) lua_pushvalue(L, -1 - nup);
  ...
  if (methods) {
    ...
    for (i = 0; i < nup; i++) lua_pushvalue(L, -2 - nup);
...
#define UNIXL_MAGIC_INITIALIZER \
  {                             \
    0, { 0 }                    \
  }
...
static int unixL_init(lua_State *L, unixL_State *U) {
  ...
  if ((error = u_pipe(U->ts.fd, O_NONBLOCK | U_CLOEXEC))) return error;
  ...
  if (KERN_SUCCESS !=
      host_get_clock_service(U->tm.host, SYSTEM_CLOCK, &U->tm.clock))
    return ENOTSUP;
#else
  if (KERN_SUCCESS != mach_timebase_info(&U->tm.timebase)) return ENOTSUP;
#endif
#endif
...
static unixL_State *unixL_loadstate(lua_State *L) {
  ...
  if (LUA_TNIL == lua_rawgetp(L, LUA_REGISTRYINDEX, &cachekey)) {
    ...
    if (lua_type(L, -1) != LUA_TUSERDATA)
      return luaL_error(
                 L, "bad cached unix state context (expected userdata, got %s)",
                 luaL_typename(L, -1)),
             (void *)NULL;
...
static u_error_t unixL_getsockname(lua_State *L, int fd,
                                   int (*getname)(int, struct sockaddr *,
                                                  socklen_t *)) {
   ...
   */
  do {
    if (salen > INT_MAX - 1) return ERANGE;
...
#endif
    if (0 != getname(fd, (struct sockaddr *)U->buf, &salen)) return errno;
#if HAVE_SOCKADDR_SA_LEN
...
#else
static uint32_t unixL_random(lua_State *L NOTUSED) { return arc4random(); }
#endif

#define r_char(charmap, maplen, r) \
  ((charmap) ? (charmap)[(0xff & (r)) % (maplen)] : (0xff & (r)))

static void unixL_random_buf(lua_State *L, void *buf, size_t bufsiz,
                             const unsigned char *charmap, size_t mapsiz) {
...
#define HAVE_MTSAFE_STRSIGNAL_                                          \
  (__sun || GLIBC_PREREQ(0, 0) || MUSL_MAYBE || FREEBSD_PREREQ(8, 1) || \
   __APPLE__ || _AIX)
...
static const char *unixL_strsignal(lua_State *L, int signo) {
  ...
  if (signo >= 0 && signo < NSIG && (info = strsignal(signo))) return info;
#elif HAVE_SYS_SIGLIST
#if !HAVE_DECL_SYS_SIGLIST
  extern const char *sys_siglist[];
#endif
  if (signo >= 0 && signo < NSIG && (info = sys_siglist[signo])) return info;
#endif
...
static const char *unixL_strerror3(lua_State *L, unixL_State *U, int error) {
  if (0 != u_strerror_r(error, U->text, sizeof U->text) || U->text[0] == '\0') {
    if (0 > snprintf(U->text, sizeof U->text, "%s: %d",
                     ((error) ? "Unknown error" : "Undefined error"), error))
      luaL_error(L, "snprintf failure");
  }
...
#define LUA_NUMBER_MAX_EXP                                    \
  ((sizeof(lua_Number) == sizeof(long double)) ? LDBL_MAX_EXP \
   ...
   : (sizeof(lua_Number) == sizeof(float))     ? FLT_MAX_EXP  \
                                               : 0)
#endif
#endif
...
static _Bool unixL_integertonumber(unixL_Integer i, lua_Number *p) {
  if (U_ISTFLOAT(lua_Number)) {
    u_static_assert(FLT_RADIX == 2, "FLT_RADIX value unsupported");
    u_static_assert(
        LUA_NUMBER_MAX_EXP == 0 || UNIXL_INTEGER_PREC < LUA_NUMBER_MAX_EXP,
        "LUA_NUMBER_MAX_EXP too small");

    if (i != (unixL_Integer)(lua_Number)i) return 0;
  } else {
    if (i > UNIXL_INTNUM_MAX || i < UNIXL_INTNUM_MIN) return 0;
  }
...
static _Bool unixL_unsignedtonumber(unixL_Unsigned i, lua_Number *p) {
  if (U_ISTFLOAT(lua_Number)) {
    u_static_assert(FLT_RADIX == 2, "FLT_RADIX value unsupported");
    u_static_assert(
        LUA_NUMBER_MAX_EXP == 0 || UNIXL_UNSIGNED_PREC < LUA_NUMBER_MAX_EXP,
        "LUA_NUMBER_MAX_EXP too small");

    if (i != (unixL_Unsigned)(lua_Number)i) return 0;
  } else {
    if (i > UNIXL_INTNUM_MAX) return 0;
  }
...
static _Bool unixL_numbertointeger(lua_Number n, unixL_Integer *p) {
  if (U_ISTFLOAT(lua_Number)) {
     ...
     */
    u_static_assert(-UNIXL_INTEGER_MAX > UNIXL_INTEGER_MIN,
                    "unixL_Integer type not two's complement");

    if (n < (lua_Number)UNIXL_INTEGER_MIN) return 0;
    if (n >= -(lua_Number)UNIXL_INTEGER_MIN) return 0;
  } else {
    if (n < UNIXL_INTEGER_MIN) return 0;
    if (n > UNIXL_INTEGER_MAX) return 0;
  }
...
static _Bool unixL_numbertounsigned(lua_Number n, unixL_Unsigned *p) {
  if (U_ISTFLOAT(lua_Number)) {
    ...
    if (n >= ldexp(1.0, UNIXL_UNSIGNED_PREC)) return 0;
  } else {
    if (n < 0) return 0;
    if (n > UNIXL_UNSIGNED_MAX) return 0;
  }
...
static void unixL_pushinteger(lua_State *L, unixL_Integer i) {
  ...
  if (unixL_integertonumber(i, &n)) {
    ...
    luaL_error(L,
               "integer value not representable as lua_Integer or lua_Number");
  }
} /* unixL_pushinteger() */
...
static void unixL_pushunsigned(lua_State *L, unixL_Unsigned i) {
  ...
  if (unixL_unsignedtonumber(i, &n)) {
    ...
    luaL_error(L,
               "unsigned integer value not representable as lua_Integer or "
               "lua_Number");
  }
} /* unixL_pushunsigned() */

#define unixL_checkinteger_(L, index, min, max, ...) \
  unixL_checkinteger((L), (index), (min), (max))
#define unixL_checkinteger(...) \
  unixL_checkinteger_(__VA_ARGS__, UNIXL_INTEGER_MIN, UNIXL_INTEGER_MAX, 0)

static unixL_Integer(unixL_checkinteger)(lua_State *L, int index,
                                         unixL_Integer min, unixL_Integer max) {
  if (lua_isinteger(L, index)) {
    lua_Integer i = lua_tointeger(L, index);

    if (i < min || i > max) goto erange;

    return i;
  } else {
    unixL_Integer i;

    if (!unixL_numbertointeger(luaL_checknumber(L, index), &i)) goto erange;
...
#define unixL_checkunsigned_(L, index, min, max, ...) \
  unixL_checkunsigned((L), (index), (min), (max))
#define unixL_checkunsigned(...) \
  unixL_checkunsigned_(__VA_ARGS__, 0, UNIXL_UNSIGNED_MAX, 0)

static unixL_Unsigned(unixL_checkunsigned)(lua_State *L, int index,
                                           unixL_Unsigned min,
                                           unixL_Unsigned max) {
  if (lua_isinteger(L, index)) {
    lua_Integer i = lua_tointeger(L, index);
    ...
    if (i < 0 || i > UNIXL_UNSIGNED_MAX) goto erange;

    U_WARN_POP;

    if ((unixL_Unsigned)i < min || (unixL_Unsigned)i > max) goto erange;

    return i;
  } else {
    unixL_Unsigned i;

    if (!unixL_numbertounsigned(luaL_checknumber(L, index), &i)) goto erange;
...
static unixL_Integer unixL_optinteger(lua_State *L, int index,
                                      unixL_Integer def, unixL_Integer min,
                                      unixL_Integer max) {
...
static off_t unixL_checkoff(lua_State *L, int index) {
  return unixL_checkinteger(L, index, MAX(UNIXL_INTEGER_MIN, U_TMIN(off_t)),
                            MIN(UNIXL_INTEGER_MAX, U_TMAX(off_t)));
} /* unixL_checkoff() */
...
static struct iovec unixL_checkstring(lua_State *L, int index, size_t min,
                                      size_t max) {
  struct iovec iov;
...
static struct sockaddr *unixL_newsockaddr(lua_State *L, const void *addr,
                                          size_t addrlen) {
  void *ud;
  ...
  null:
    *len = 0;
    ...
    if (LUA_TSTRING != lua_getfield(L, index, "path"))
      return luaL_error(L, "no path specified for AF_UNIX address"),
             (void *)NULL;
    ...
    *len = addrlen;
    ...
    u_freeaddrinfo(&U->net.res);
    error = getaddrinfo(lua_tostring(L, -2), lua_tostring(L, -1), &hints,
                        &U->net.res);
    if (error) {
      U->net.res = NULL;
      goto null;
...
static struct sockaddr *unixL_checksockaddr(lua_State *L, int index,
                                            size_t *len) {
  struct sockaddr *sa;

  if (!(sa = unixL_tosockaddr(L, index, len)))
    luaL_error(L, "expected struct sockaddr, got %s",
               lua_typename(L, lua_type(L, index)));

  return sa;
} /* unixL_checksockaddr() */

static int unixL_pusherror(lua_State *L, int error, const char *fun NOTUSED,
                           const char *fmt) {
...
static u_error_t fd_reopen(int *fd, int ofd, const char *fspath,
                           u_flags_t flags) {
  ...
  if ((dev && (!nul || dev < nul)) || (ino && (!nul || ino < nul))) {
    if (0 != fstat(ofd, &st)) return errno;
  }

  if ((error = u_snprintf(path, sizeof path, fspath, (long)getpid(), ofd,
                          (long long)st.st_dev, (long long)st.st_ino, '\0')))
    return error;

  if ((error = u_getaccmode(ofd, &flags, flags))) return error;

  if (-1 == (*fd = open(path, U_SYSFLAGS & flags))) return errno;
...
static u_error_t fd_reopendir(int *fd, int ofd, u_flags_t flags) {
  ...
  if (-1 == (*fd = openat(ofd, ".", U_SYSFLAGS & flags))) return errno;
...
static u_error_t fd_isdiff(int *diff, int fd1, int fd2, int flag) {
  ...
  *diff = 0;
   ...
   */
  if (-1 == (flags1 = fcntl(fd1, F_GETFL))) return errno;
  if (-1 == (flags2 = fcntl(fd2, F_GETFL))) return errno;

  if ((*diff = (flags1 & flag) ^ (flags2 & flag))) return 0;

  if (0 != fcntl(fd1, F_SETFL, flags1 | flag)) return errno;
...
static u_error_t fd_mktemp(lua_State *L, int *fd, char *tmpnam, size_t tmpsiz,
                           u_flags_t flags, mode_t mode) {
  ...
  for (i = 0; i < countof(tmpdir); i++) {
    if (!tmpdir[i] && !(tmpdir[i] = getenv("TMPDIR"))) continue;

    for (j = 0; j < i; j++) {
      if (tmpdir[j] && 0 == strcmp(tmpdir[i], tmpdir[j])) goto next;
    }

    if ((error =
             u_snprintf(tmpnam, tmpsiz, "%s/luaunix.%s", tmpdir[i], tmpext)))
      continue;

    if ((error = u_open(fd, tmpnam, flags | O_CREAT | O_EXCL, mode))) continue;
...
static u_error_t fd_init(lua_State *L, unixL_State *U) {
  static const char *const paths[] = {"/proc/%" FD_PRIpid "/fd/%" FD_PRIfd
                                      "%" FD_PRInul "%" FD_PRIdev "%" FD_PRIino,
                                      "/dev/fd/%" FD_PRIfd "%" FD_PRInul
                                      "%" FD_PRIpid "%" FD_PRIdev "%" FD_PRIino,
                                      "/.vol/%" FD_PRIdev "/%" FD_PRIino
                                      "%" FD_PRInul "%" FD_PRIfd "%" FD_PRIpid};
  const char *const *path;
  int pipefd[2] = {-1, -1}, tmpfd = -1, fd = -1, diff, error;
  char tmpnam[256];

  if (*U->fd.path) return 0;
  if (U->fd.error) return U->fd.error;

  /*
   * Try pipe first because it doesn't require touching the file
   * system.
   */
  if ((error = u_pipe(pipefd, U_CLOEXEC))) goto error;

  for (path = paths; path < endof(paths); u_close(&fd), path++) {
    if ((error = fd_reopen(&fd, pipefd[0], *path, O_RDONLY | U_CLOEXEC)))
      continue;
    if ((error = fd_isdiff(&diff, fd, pipefd[0], O_NONBLOCK))) goto error;
    if (!diff) continue;

    if ((error = u_strcpy(U->fd.path, *path, sizeof U->fd.path))) goto error;
   ...
   */
  if ((error = fd_mktemp(L, &tmpfd, tmpnam, sizeof tmpnam,
                         O_WRONLY | O_APPEND | U_CLOEXEC, 0600)))
    goto error;

  for (path = paths; path < endof(paths); u_close(&fd), path++) {
    if ((error = fd_reopen(&fd, tmpfd, *path, O_WRONLY | U_CLOEXEC))) continue;
    if ((error = fd_isdiff(&diff, fd, tmpfd, O_APPEND))) goto error;
    if (!diff) continue;
...
static u_error_t unixL_reopen(lua_State *L, int *fd, int ofd, u_flags_t flags) {
  ...
  *fd = -1;
  ...
  if ((error = fd_init(L, U))) goto error;

  if ((error = fd_reopen(fd, ofd, U->fd.path, flags))) goto error;

  return 0;
error:
  if (0 == fd_reopendir(fd, ofd, flags)) return 0;
...
static u_error_t unixL_closedir(lua_State *L, DIR **dp) {
  ...
  if (*dp) {
    if (U->dir.dp == *dp) U->dir.dp = NULL;

    if (0 != closedir(*dp)) error = errno;
...
static int unixL_getpwnam(lua_State *L, const char *user, struct passwd **ent) {
  ...
  *ent = NULL;
  ...
  while (
      (error = u_getpwnam_r(user, &U->pw.ent, U->pw.buf, U->pw.bufsiz, ent))) {
    if (error != ERANGE) return error;

    if ((error = u_realloc(&U->pw.buf, &U->pw.bufsiz, 128))) return error;
...
static int unixL_getpwuid(lua_State *L, uid_t uid, struct passwd **ent) {
  ...
  *ent = NULL;
  ...
  while (
      (error = u_getpwuid_r(uid, &U->pw.ent, U->pw.buf, U->pw.bufsiz, ent))) {
    if (error != ERANGE) return error;
...
static int unixL_getgrnam(lua_State *L, const char *group, struct group **ent) {
  ...
  *ent = NULL;
  ...
  while (
      (error = u_getgrnam_r(group, &U->gr.ent, U->gr.buf, U->gr.bufsiz, ent))) {
    if (error != ERANGE) return error;

    if ((error = u_realloc(&U->gr.buf, &U->gr.bufsiz, 128))) return error;
...
static int unixL_getgrgid(lua_State *L, gid_t gid, struct group **ent) {
  ...
  *ent = NULL;
  ...
  while (
      (error = u_getgrgid_r(gid, &U->gr.ent, U->gr.buf, U->gr.bufsiz, ent))) {
    if (error != ERANGE) return error;
...
static uid_t unixL_optuid(lua_State *L, int index, uid_t def) {
  ...
  if (!pw) return luaL_error(L, "%s: no such user", user);
...
static gid_t unixL_optgid(lua_State *L, int index, gid_t def) {
  ...
  if (!gr) return luaL_error(L, "%s: no such group", group);
...
MAYBEUSED static int pr_psinfo(struct psinfo *pr) {
  ...
  if ((error =
           u_snprintf(path, sizeof path, "/proc/%ld/psinfo", (long)getpid())))
    goto error;

  if ((error = u_open(&fd, path, O_RDONLY | U_CLOEXEC, 0))) goto error;
...
static int ts_nthreads_psinfo(void) {
  ...
  if ((error = pr_psinfo(&pr))) return -1;

  if (pr.pr_nlwp > INT_MAX) return -1;
...
static int ts_nthreads_pstatus(void) {
  ...
  if (SIZEOF_STRUCT_PSTATUS_PR_NLWP != sizeof pr_nlwp) goto oops;

  if (0 > snprintf(path, sizeof path, "/proc/%ld/status", (long)getpid()))
    goto oops;

  if (0 != u_open(&fd, path, O_RDONLY | U_CLOEXEC, 0)) goto oops;

  if (sizeof data != read(fd, data, sizeof data)) goto oops;

  u_close(&fd);

  memcpy(&pr_nlwp, &data[OFFSETOF_STRUCT_PSTATUS_PR_NLWP],
         SIZEOF_STRUCT_PSTATUS_PR_NLWP);
...
MAYBEUSED static int ts_nthreads(void) {
  ...
  if ((n = ts_nthreads_psinfo()) > 0) return n;
  if ((n = ts_nthreads_pstatus()) > 0) return n;
...
static mode_t unixL_getumask(lua_State *L) {
  ...
  switch ((pid = fork())) {
    ...
    case 0:
      ...
      if (sizeof mask != write(U->ts.fd[1], &mask, sizeof mask)) _Exit(1);
    ...
    default:
      ...
      if (sizeof mask != (n = read(U->ts.fd[0], &mask, sizeof mask)))
        return luaL_error(L, "getumask: %s",
                          (n == -1) ? unixL_strerror(L, errno) : "short read");
    ...
    for (; *fmt; ++fmt) {
      switch (*fmt) {
        ...
        case 'x':
          ...
          continue;
        case 'X':
          if (S_ISDIR(omode) || (omode & 00111)) perm |= 00111;
        ...
        case 'o':
          ...
          continue;
        default:
          if (isspace((unsigned char)*fmt)) continue;
...
static int unixL_xoptfileno(lua_State *L, int index, int def, _Bool atok) {
  ...
  if ((fh = luaL_testudata(L, index, LUA_FILEHANDLE))) {
    ...
    luaL_argcheck(L, fd >= 0, index,
                  "attempt to use irregular file (no descriptor)");
  ...
  if ((dp = luaL_testudata(L, index, "DIR*"))) {
    ...
    luaL_argcheck(L, fd >= 0, index,
                  "attempt to use irregular directory (no descriptor)");
  ...
  if (lua_type(L, index) == LUA_TNUMBER) {
    ...
    if (fd < 0 && !(atok && u_isatfd(fd)))
      luaL_argcheck(L, 0, index,
                    lua_pushfstring(L, "bad file descriptor (%d)", fd));
...
static int unixL_checkatfileno(lua_State *L, int index) {
  ...
  luaL_argcheck(L, fd >= 0 || u_isatfd(fd), index,
...
static struct luaL_Stream *unixL_prepfile(lua_State *L) {
  ...
  if (U->lua.jit) {
    ...
    if (U->lua.openf == LUA_NOREF || U->lua.openf == LUA_REFNIL)
      luaL_error(L,
                 "unable to create new file handle: LuaJIT io.open function "
                 "not available");
    ...
    if (lua_isnil(L, -2))
      luaL_error(L, "unable to create a new file handle: %s: %s", *ppath,
...
static void unixL_checkflags(lua_State *L, int index, const char **mode,
                             u_flags_t *flags, mode_t *perm) {
  index = lua_absindex(L, index);
...
static struct tm *unixL_opttm(lua_State *L, int index, const struct tm *def,
                              struct tm *tm) {
...
static int unixL_optclockid(lua_State *L, int index, int def) {
  ...
  if (lua_isnoneornil(L, index)) return def;
  if (lua_isnumber(L, index)) return luaL_checkint(L, index);
  ...
  switch (*((*id == '*') ? id + 1 : id)) {
    ...
    default:
      return luaL_argerror(L, index,
                           lua_pushfstring(L, "%s: invalid clock", id));
  }
} /* unixL_optclockid() */
...
static int unixL_getenv(lua_State *L, int index) {
  ...
  if (!(value = getenv(luaL_checkstring(L, index)))) return 0;
...
static int unixL_setenv(lua_State *L, int nameindex, int valueindex,
                        int chgindex) {
...
static sigset_t *unixL_tosigset(lua_State *L, int index, sigset_t *buf) {
  ...
  if ((set = luaL_testudata(L, index, "sigset_t"))) return set;
...
static const luaL_Reg sigset_methods[] = {{NULL, NULL}}; /* sigset_methods[] */

static const luaL_Reg sigset_metamethods[] = {
    {NULL, NULL}}; /* sigset_metamethods[] */
...
static const luaL_Reg sighandler_metamethods[] = {
    {"__eq", &sighandler__eq}, {NULL, NULL}}; /* sighandler_metamethods[] */
...
static int unix_accept(lua_State *L) {
  ...
  if (U->bufsiz < sizeof(struct sockaddr_storage) &&
      (error = u_realloc(&U->buf, &U->bufsiz, sizeof(struct sockaddr_storage))))
...
#endif
  if (U->net.fd == -1) goto syerr;

  lua_pushinteger(L, U->net.fd);
  if (salen <= U->bufsiz) {
    unixL_newsockaddr(L, U->buf, salen);
  } else {
    if ((error = unixL_getsockname(L, U->net.fd, &getpeername))) goto error;
  }
  U->net.fd = -1;
...
#if HAVE_ARC4RANDOM
#if ((__APPLE__ && !MACOS_PREREQ(10, 12, 0)) ||  \
     (__FreeBSD__ && !FREEBSD_PREREQ(10, 0))) && \
...
static int unix_chdir(lua_State *L) {
  ...
  if (-1 != (fd = unixL_optfileno(L, 1, -1))) {
    if (0 != fchdir(fd)) return unixL_pusherror(L, errno, "chdir", "0$#");
  } else {
    const char *path = luaL_checkstring(L, 1);

    if (0 != chdir(path)) return unixL_pusherror(L, errno, "chdir", "0$#");
  }
...
static int unix_chmod(lua_State *L) {
  ...
  if (-1 != (fd = unixL_optfileno(L, 1, -1))) {
    if (!octal) {
      if (0 != fstat(fd, &st)) return unixL_pusherror(L, errno, "chmod", "0$#");
    ...
    if (0 != fchmod(fd, mode)) return unixL_pusherror(L, errno, "chmod", "0$#");
  } else {
    const char *path = luaL_checkstring(L, 1);
...
static int unix_chroot(lua_State *L) {
  ...
  if (0 != chroot(path)) return unixL_pusherror(L, errno, "chroot", "0$#");
...
static u_error_t exec_addarg(unixL_State *U, size_t *arrp, const char *s) {
  ...
  if ((error =
           u_reallocarray_char_pp(&U->exec.arr, &U->exec.arrsiz, (*arrp) + 1)))
    return error;
...
static u_error_t exec_addtable(lua_State *L, unixL_State *U, size_t *arrp,
                               int index, int anchorindex) {
  size_t i = 0;
  int error = 0;

  IPAIRS_BEGIN(L, index);

  if (i++ >= INT_MAX) IPAIRS_STOP(L);

  if ((error = exec_addarg(U, arrp, luaL_checkstring(L, -1)))) IPAIRS_STOP(L);
...
static int unix_execve(lua_State *L) {
  ...
  if (!lua_isnil(L, 2)) {
    if ((error = exec_addtable(L, U, &arrp, 2, 4))) goto error;
  }

  argc = arrp;

  if ((error = exec_addarg(U, &arrp, NULL))) goto error;

  if (!lua_isnil(L, 3)) {
    if ((error = exec_addtable(L, U, &arrp, 3, 4))) goto error;
  }
...
static int unix_execl(lua_State *L) {
  ...
  for (i = 2; i <= top; i++) {
    if ((error = exec_addarg(U, &arrp, luaL_checkstring(L, i)))) goto error;
  }
...
static int unix_execvp(lua_State *L) {
  ...
  if (!lua_isnil(L, 2)) {
    if ((error = exec_addtable(L, U, &arrp, 2, 3))) goto error;
  }
...
static int unix_fcntl(lua_State *L) {
  ...
  switch (cmd) {
...
#endif
    case F_DUPFD:
      if (-1 == (dupfd = fcntl(fd, cmd, unixL_checkfileno(L, 3)))) goto syerr;

      unixL_pushinteger(L, dupfd);

      return 1;
    case F_GETFD:
      if ((flags = fcntl(fd, cmd)) < 0) goto syerr;

      unixL_pushinteger(L, flags);

      return 1;
    case F_GETFL:
      if ((error = u_getflags(fd, &flags))) goto error;

      unixL_pushinteger(L, flags);

      return 1;
    case F_SETFD:
      if (-1 == fcntl(fd, cmd, (int)luaL_checkint(L, 3))) goto syerr;
    ...
    case F_SETFL:
      ...
      if (-1 == fcntl(fd, cmd, (int)(U_SYSFLAGS & flags))) goto syerr;

      if (flags & U_CLOEXEC) {
        if (-1 == fcntl(fd, F_SETFD, FD_CLOEXEC)) goto syerr;
      }
      ...
      return 1;
    case F_GETOWN:
      if (-1 == (pid = fcntl(fd, cmd))) goto syerr;
    ...
    case F_SETOWN:
      ...
      if (-1 == fcntl(fd, cmd, pid)) goto syerr;
    ...
    case F_MAXFD: {
      ...
      if (-1 == (maxfd = fcntl(fd, cmd))) goto syerr;
    ...
    case F_GETPATH: {
      ...
      if (-1 == fcntl(fd, cmd, U->buf)) goto syerr;
...
static int unsafe_fcntl(lua_State *L) {
  ...
  n = lua_gettop(L);
  luaL_argcheck(L, n <= 3, 4,
                lua_pushfstring(L, "expected 3 arguments, got %d", n));

  switch (lua_type(L, 3)) {
    case LUA_TNONE: {
      if (-1 == (r = fcntl(fd, cmd, (intptr_t)0))) goto syerr;
    ...
    case LUA_TNUMBER: {
      ...
      if (-1 == (r = fcntl(fd, cmd, arg))) goto syerr;
    ...
    case LUA_TSTRING: {
      ...
      if (U->bufsiz < iov.iov_len &&
          (error = u_realloc(&U->buf, &U->bufsiz, iov.iov_len)))
        goto error;
      memcpy(U->buf, iov.iov_base, iov.iov_len);

      if (-1 == (r = fcntl(fd, cmd, U->buf))) goto syerr;
  ...
  return luaL_argerror(L, 3,
                       lua_pushfstring(L, "expected integer or string, got %s",
                                       luaL_typename(L, 3)));
syerr:
  error = errno;
error:
...
static int unix_fdatasync(lua_State *L) {
  ...
  if (0 != fdatasync(fd)) return unixL_pusherror(L, errno, "fdatasync", "0$#");
...
static int unix_fdopen(lua_State *L) {
  ...
  luaL_argcheck(L, lua_type(L, 1) != LUA_TUSERDATA, 1,
                "cannot steal descriptor from existing handle");
...
static int unix_fdup(lua_State *L) {
  ...
  if ((error = u_dup(&fd, ofd, flags))) goto error;

  if ((error = u_fdopen(&fh->f, &fd, mode, flags))) goto error;
...
static int unix_fgetc(lua_State *L) {
  ...
  if (EOF == (c = fgetc(fp))) {
    if (ferror(fp)) return unixL_pusherror(L, errno, "fgetc", "~$#");
...
static int unix_fsync(lua_State *L) {
  ...
  if (0 != fsync(fd)) return unixL_pusherror(L, errno, "fsync", "0$#");
...
static int unix_fopen(lua_State *L) {
  ...
  if (-1 != (ofd = unixL_optfileno(L, 1, -1))) {
    if ((error = unixL_reopen(L, &fd, ofd, flags))) goto error;
    ...
    if (mode) {
      if (!(fh->f = fopen(path, mode))) goto syerr;
    } else {
      if ((error = u_open(&fd, path, flags, perm))) goto error;

      if (!(fh->f = fdopen(fd, u_strmode(flags)))) goto syerr;
...
static int unix_fopenat(lua_State *L) {
  ...
  fh = unixL_prepfile(L);
  if (-1 == (fd = openat(at, path, flags, perm))) goto syerr;
...
static int unix_fpipe(lua_State *L) {
  ...
  if ((error = u_pipe(fd, flags))) goto error;

  if ((error = u_fdopen(&fh[0]->f, &fd[0], mode, flags | O_RDONLY))) goto error;
  if ((error = u_fdopen(&fh[1]->f, &fd[1], mode, flags | O_WRONLY))) goto error;
...
static int unix_fork(lua_State *L) {
  ...
  if (-1 == (pid = fork())) return unixL_pusherror(L, errno, "fork", "~$#");
...
static const char *gai_field[] = {"family",    "socktype", "protocol", "addr",
                                  "canonname", "port",     NULL};

static int gai_pushaddr(lua_State *L, struct sockaddr *addr,
                        socklen_t addrlen) {
  char host[NI_MAXHOST + 1];
  int error;

  if ((error = getnameinfo(addr, addrlen, host, sizeof host, NULL, 0,
                           NI_NUMERICHOST))) {
    lua_pushnil(L);
  } else {
...
static void gai_pushfield(lua_State *L, const struct addrinfo *res,
                          enum gai_field type) {
  switch (type) {
    case GAI_FAMILY:
...
static int gai_nextai(lua_State *L) {
  ...
  if (lua_isnone(L, lua_upvalueindex(4))) {
    ...
    for (i = 4; !lua_isnone(L, lua_upvalueindex(i)); i++) {
      gai_pushfield(L, res,
                    luaL_checkoption(L, lua_upvalueindex(i), NULL, gai_field));
    }
...
static const luaL_Reg gai_methods[] = {{NULL, NULL}}; /* gai_methods[] */

static const luaL_Reg gai_metamethods[] = {
    {"__gc", &gai__gc}, {NULL, NULL}}; /* gai_metamethods[] */
...
static int unix_getcwd(lua_State *L) {
  ...
  while (!(path = getcwd(U->buf, U->bufsiz))) {
    if (errno != ERANGE) goto syerr;
    if ((error = u_growby(&U->buf, &U->bufsiz, 255))) goto error;
  }
...
static void gr_pushmem(lua_State *L, char **list, int create) {
  if (list) {
    ...
    if (create) lua_createtable(L, i, 0);
    ...
    if (create) lua_createtable(L, 0, 0);
  }
} /* gr_pushmem() */
...
static const char *ifs_field[] = {"name",      "flags",     "addr", "netmask",
                                  "dstaddr",   "broadaddr", "data", "family",
                                  "prefixlen", NULL};
...
static int ifs_pushaddr(lua_State *L, struct sockaddr *sa) {
  ...
  if ((error = getnameinfo(sa, u_sa_len(sa), host, sizeof host, NULL, 0,
                           NI_NUMERICHOST))) {
...
static void ifs_pushfield(lua_State *L, const struct u_ifaddrs *ifa,
                          enum ifs_field type) {
  switch (type) {
    case IF_NAME:
...
static int ifs_nextif(lua_State *L) {
  ...
  if (lua_isnone(L, lua_upvalueindex(3))) {
    ...
    for (i = 3; !lua_isnone(L, lua_upvalueindex(i)); i++) {
      ifs_pushfield(L, ifa,
                    luaL_checkoption(L, lua_upvalueindex(i), NULL, ifs_field));
    }
...
static const luaL_Reg ifs_methods[] = {{NULL, NULL}}; /* ifs_methods[] */

static const luaL_Reg ifs_metamethods[] = {
    {"__gc", &ifs__gc}, {NULL, NULL}}; /* ifs_metamethods[] */
...
static int unix_getnameinfo(lua_State *L) {
  ...
  if ((error =
           getnameinfo(sa, salen, host, sizeof host, serv, sizeof serv, flags)))
    return gai_pusherror(L, error);
...
static int getopt_pushargs(lua_State *L, int index) {
  ...
  for (i = arg0, argc = 0, isnil = 0; i < SIZE_MAX && !isnil;
       i++, argc += !isnil) {
...
static int unix_getprogname(lua_State *L) {
  ...
  if (!(path = getexecname())) goto notsup;
  ...
  extern char **p_xargv;
#endif
  if (!*p_xargv) goto notsup;

  name = getprogname_basename(*p_xargv);
#endif

  if (!name || !*name) goto notsup;
...
static int rl_checkrlimit(lua_State *L, int index) {
  ...
  if (lua_isnumber(L, index)) return unixL_checkint(L, index);

  i = luaL_checkoption(L, index, NULL, what_s);
  luaL_argcheck(
      L, i >= 0 && i < (int)countof(what_i), index,
      lua_pushfstring(L, "unexpected rlimit (%s)", lua_tostring(L, index)));
...
static rlim_t rl_checkrlim(lua_State *L, int index) {
  ...
  if (!lua_isinteger(L, index)) {
     ...
     */
    if (rl_isequal(L, index, RL_RLIM_INFINITY)) return U_RLIM_INFINITY;
    if (rl_isequal(L, index, RL_RLIM_SAVED_CUR)) return U_RLIM_SAVED_CUR;
    if (rl_isequal(L, index, RL_RLIM_SAVED_MAX)) return U_RLIM_SAVED_MAX;
  }
...
static int ru_checkrusage(lua_State *L, int index) {
  ...
  luaL_argcheck(
      L, i >= 0 && i < (int)countof(what_i), index,
      lua_pushfstring(L, "unexpected resource (%s)", lua_tostring(L, index)));
...
static int unsafe_getsockopt(lua_State *L) {
  ...
  const struct iovec iov =
      unixL_checkstring(L, 4, 0, INT_MAX); /* length must fit socklen_t */
  socklen_t bufsiz;
  int n, error;

  n = lua_gettop(L);
  luaL_argcheck(L, n <= 4, 5,
                lua_pushfstring(L, "expected 4 arguments, got %d", n));
  ...
  if (0 != getsockopt(fd, level, type, U->buf, &bufsiz)) goto syerr;
...
static int unix_grantpt(lua_State *L) {
  ...
  if (0 != grantpt(fd)) return unixL_pusherror(L, errno, "grantpt", "~$#");
...
static int unix_ioctl(lua_State *L) {
  ...
  switch (cmd) {
    ...
    case SIOCATMARK:
      if (-1 == ioctl(fd, cmd, &val)) goto syerr;
    ...
    case TIOCNOTTY:
      if (-1 == ioctl(fd, cmd, (char *)NULL)) goto syerr;
...
static int unsafe_ioctl(lua_State *L) {
  ...
  switch (lua_type(L, 3)) {
    case LUA_TNONE: {
      if (-1 == (r = ioctl(fd, cmd, (intptr_t)0))) goto syerr;
    ...
    case LUA_TNUMBER: {
      ...
      if (-1 == (r = ioctl(fd, cmd, arg))) goto syerr;
    ...
    case LUA_TSTRING: {
      ...
      if (-1 == (r = ioctl(fd, cmd, U->buf))) goto syerr;
...
static int unix_link(lua_State *L) {
  ...
  if (0 != link(src, dst)) return unixL_pusherror(L, errno, "link", "0$#");
...
static int unix_lstat(lua_State *L) {
  ...
  if (0 != lstat(path, &st)) return unixL_pusherror(L, errno, "lstat", "0$#");
...
static int unsafe_mlock(lua_State *L) {
  ...
  if (0 != mlock(addr, len)) return unixL_pusherror(L, errno, "mlock", "0$#");
...
static int unsafe_mlockall(lua_State *L) {
  ...
  if (0 != mlockall(flags)) return unixL_pusherror(L, errno, "mlockall", "0$#");
...
static int mman_optfileno(lua_State *L, int index, int def) {
  if (lua_type(L, index) == LUA_TNUMBER) return unixL_checkint(L, index);
...
static int unsafe_munlockall(lua_State *L) {
  if (0 == munlockall()) return unixL_pusherror(L, errno, "munlockall", "0$#");
...
static int unsafe_munmap(lua_State *L) {
  ...
  if (0 != munmap(addr, len)) return unixL_pusherror(L, errno, "munmap", "~$#");
...
enum dir_field { DF_NAME, DF_INO, DF_TYPE }; /* enum dir_field */

static const char *dir_field[] = {"name", "ino", "type", NULL};

static void dir_pushfield(lua_State *L, struct dirent *ent,
                          enum dir_field type) {
  switch (type) {
    case DF_NAME:
...
static int dir_nextent(lua_State *L) {
  ...
  if (nup < 4) {
    ...
    for (i = 4; i <= nup; i++, n++) {
      dir_pushfield(L, ent,
                    luaL_checkoption(L, lua_upvalueindex(i), NULL, dir_field));
    }
...
static const luaL_Reg dir_methods[] = {{"read", &dir_read},
                                       {"files", &dir_files},
                                       {"rewind", &dir_rewind},
                                       {"close", &dir_close},
                                       {NULL, NULL}}; /* dir_methods[] */

static const luaL_Reg dir_metamethods[] = {
    {"__gc", &dir_close}, {NULL, NULL}}; /* dir_metamethods[] */
...
static int unix_opendir(lua_State *L) {
  ...
  *dp = NULL;
  ...
  if (-1 != (fd = unixL_optfileno(L, 1, -1))) {
    if ((error = unixL_reopen(L, &fd2, fd, U_CLOEXEC))) goto error;

    if (-1 == lseek(fd2, 0, SEEK_SET)) goto syerr;

    if ((error = u_fdopendir(dp, &fd2, 0))) goto error;
  } else {
    const char *path = luaL_checkstring(L, 1);

    if (!(*dp = opendir(path))) goto syerr;
  }
...
static int unix_pathconf(lua_State *L) {
  ...
  if (v == -1 && errno) return unixL_pusherror(L, errno, "pathconf", "~$#");
...
static u_error_t poll_add(unixL_State *U, int fd, short events, size_t *nfds,
                          size_t *mfds) {
  int error;

  if (*nfds >= INT_MAX) return ERANGE;

  if (*mfds <= *nfds) {
    if ((error = u_reallocarray_pollfd(&U->net.fds.buf, &U->net.fds.bufsiz,
                                       *nfds + 1)))
      return error;
...
static int unix_posix_fopenpt(lua_State *L) {
  ...
  if (-1 == (fd = posix_openpt(flags))) goto syerr;

  if ((error = u_fdopen(&fh->f, &fd, NULL, flags))) goto error;
...
static int unix_readlink(lua_State *L) {
  ...
  do {
    if (U->bufsiz <= (size_t)n &&
        (error = u_realloc(&U->buf, &U->bufsiz, n + 1)))
...
static int unix_realpath(lua_State *L) {
  ...
  free(path);
  if (error) return unixL_pusherror(L, error, "realpath", "~$#");
...
static int unix_recvfrom(lua_State *L) {
  ...
  fromlen = sizeof from;
  if (-1 == (n = recvfrom(fd, U->buf, size, flags, (struct sockaddr *)&from,
                          &fromlen)))
...
static int unix_recvfromto(lua_State *L) {
  ...
  tolen = sizeof to;
  if (-1 ==
      (n = u_recvfromto(fd, U->buf, size, flags, (struct sockaddr *)&from,
                        &fromlen, (struct sockaddr *)&to, &tolen, &error)))
    return unixL_pusherror(L, error, "recvfromto", "~$#");
...
static int sa__index_in(lua_State *L, const struct sockaddr_in *in,
                        const char *k) {
  if (!strcmp(k, "addr")) {
    char addr[INET_ADDRSTRLEN];

    if (!inet_ntop(AF_INET, &in->sin_addr, addr, sizeof addr)) return 0;
...
static int sa__index_in6(lua_State *L, const struct sockaddr_in6 *in6,
                         const char *k) {
  if (!strcmp(k, "addr")) {
    char addr[INET6_ADDRSTRLEN];

    if (!inet_ntop(AF_INET6, &in6->sin6_addr, addr, sizeof addr)) return 0;
    lua_pushstring(L, addr);
    return 1;
  } else if (!strcmp(k, "port")) {
...
static int sa__index_un(lua_State *L, const struct sockaddr_un *un,
                        size_t unlen, const char *k) {
  if (!strcmp(k, "path")) {
    size_t pathsiz, pathlen;

    if (unlen < offsetof(struct sockaddr_un, sun_path)) return 0;
...
static const luaL_Reg sa_metamethods[] = {{"__index", &sa__index},
                                          {"__tostring", &sa__tostring},
                                          {NULL, NULL}}; /* sa_metamethods[] */
...
static size_t regcomp_nsub(const char *cp, const int cflags) {
  const char *obp = NULL;
  int state = 0, ch;
  size_t n = 0;
...
static struct u_regex *regcomp_prepregex(lua_State *L, int index, size_t nsub) {
  struct u_regex *re;
  size_t size;

  /* +1 for 0th match */
  if (nsub + 1 >
      (SIZE_MAX - offsetof(struct u_regex, match)) / sizeof re->match[0])
...
static int regex_pusherrstr(lua_State *L, int error, regex_t *preg) {
  luaL_Buffer errbuf;
  size_t n;
...
static int regex_pusherror(lua_State *L, int error, regex_t *preg) {
  lua_pushnil(L);
...
static int regex_pushmatch(lua_State *L, const char *s, regmatch_t rm) {
...
static struct u_regex *regex_checkself(lua_State *L, int index) {
  struct u_regex *re = luaL_checkudata(L, index, "regex_t");

  luaL_argcheck(L, !re->closed, index,
                "attempt to use freed regular expression");

  return re;
}

static int regex__index(lua_State *L) {
...
static int regex__tostring(lua_State *L) {
  struct u_regex *re = regex_checkself(L, 1);
...
static int regex__gc(lua_State *L) {
...
static int unix_regcomp(lua_State *L) {
  ...
  return luaL_error(L,
                    "unable to preallocate match array for regular expression");
} /* unix_regcomp() */

static int unix_regerror(lua_State *L) {
  int error = unixL_checkint(L, 1);
  struct u_regex *re =
      (lua_isnoneornil(L, 2)) ? NULL : luaL_checkudata(L, 2, "regex_t");
...
static int unix_regexec(lua_State *L) {
  ...
  if (top > 2 && lua_istable(L, top)) {
    luaL_argcheck(
        L, !(re->cflags & REG_NOSUB), top,
        "match array specified but regular expression compiled with REG_NOSUB");
    tindex = top--;
  }
  ...
  luaL_argcheck(L, top == 2, top,
                lua_pushfstring(L, "expected integer or table, got %s",
                                luaL_typename(L, top)));

  if ((error =
           regexec(&re->regex, subj, re->regex.re_nsub + 1, re->match, eflags)))
    return regex_pusherror(L, error, &re->regex);
  ...
  if (re->cflags & REG_NOSUB) {
    ...
    if (re->regex.re_nsub > (size_t)(INT_MAX - lua_gettop(L)) ||
        !lua_checkstack(L, re->regex.re_nsub))
...
static int unix_rmdir(lua_State *L) {
  ...
  if (0 != rmdir(path)) return unixL_pusherror(L, errno, "rmdir", "0$#");
...
static int unix_sendtofrom(lua_State *L) {
  ...
  if (-1 == (n = u_sendtofrom(fd, src, size, flags, to, tolen, from, fromlen,
                              &error)))
...
static int unix_setegid(lua_State *L) {
  ...
  if (0 != setegid(gid)) return unixL_pusherror(L, errno, "setegid", "0$#");
...
static int unix_seteuid(lua_State *L) {
  ...
  if (0 != seteuid(uid)) return unixL_pusherror(L, errno, "seteuid", "0$#");
...
static int unix_setgid(lua_State *L) {
  ...
  if (0 != setgid(gid)) return unixL_pusherror(L, errno, "setgid", "0$#");
...
setbool:
  i = lua_toboolean(L, 4);
  if (0 != setsockopt(fd, level, type, &i, sizeof i)) goto syerr;
  lua_pushboolean(L, 1);
  return 1;
setint:
...
static int unix_setsid(lua_State *L) {
  ...
  if (-1 == (pg = setsid())) return unixL_pusherror(L, errno, "setsid", "~$#");
...
static int unix_setuid(lua_State *L) {
  ...
  if (0 != setuid(uid)) return unixL_pusherror(L, errno, "setuid", "0$#");
...
static int unix_sigaction(lua_State *L) {
  ...
  if (lua_isnil(L, 2)) {
    if (0 != sigaction(signo, NULL, &oact)) goto syerr;
  } else {
    ...
    if (!lua_isnil(L, -1)) act.sa_mask = *unixL_tosigset(L, -1, NULL);
    lua_pop(L, 1);

    act.sa_flags = unixL_optfint(L, 2, "flags", 0);

    if (0 != sigaction(signo, &act, &oact)) goto syerr;
  }

  if (lua_toboolean(L, 3)) {
    lua_newtable(L);

    *(u_sighandler_t **)lua_newuserdata(L, sizeof(u_sighandler_t *)) =
        (u_sighandler_t *)oact.sa_handler;
...
static int unix_sigaddset(lua_State *L) {
  ...
  for (i = 2; i <= lua_gettop(L); i++) sigaddset(set, luaL_checkint(L, i));
...
static int unix_sigdelset(lua_State *L) {
  ...
  for (i = 2; i <= lua_gettop(L); i++) sigdelset(set, luaL_checkint(L, i));
...
static int unix_sigismember(lua_State *L) {
  ...
  lua_pushboolean(L,
                  sigismember(unixL_tosigset(L, 1, &tmp), luaL_checkint(L, 2)));
...
static int unix_sigtimedwait(lua_State *L) {
  ...
  if (lua_isnoneornil(L, 1)) {
    ...
    if (&tmp != (set = unixL_tosigset(L, 1, &tmp))) tmp = *set;
  }
  ...
  if ((error = u_sigtimedwait(&signo, &tmp, &si,
                              u_f2ts(&timeout, luaL_optnumber(L, 2, U_NAN)))))
...
static int unix_sigwait(lua_State *L) {
  ...
  if (lua_isnoneornil(L, 1)) {
    ...
    if (&set != (_set = unixL_tosigset(L, 1, &set))) set = *_set;
  }
...
static const char *st_field[] = {
    ...
    "atime", "mtime", "ctime", "blksize", "blocks", NULL}; /* st_field[] */

/* AIX uses struct st_timespec, where tv_nsec is an int instead of a long. */
#define ST_TIMESPEC_TO_TIMESPEC(st_ts, ts) \
  do {                                     \
    (ts)->tv_sec = (st_ts)->tv_sec;        \
    (ts)->tv_nsec = (st_ts)->tv_nsec;      \
  } while (0)

#define st_pushtimespec(L, st_ts)          \
  do {                                     \
    struct timespec ts;                    \
...
static void st_pushfield(lua_State *L, const struct stat *st,
                         enum st_field type) {
  switch (type) {
    case STF_DEV:
...
static int unix_stat(lua_State *L) {
  ...
  if (-1 != (fd = unixL_optfileno(L, 1, -1))) {
    if (0 != fstat(fd, &st)) return unixL_pusherror(L, errno, "stat", "0$#");
  } else {
    const char *path = luaL_checkstring(L, 1);

    if (0 != stat(path, &st)) return unixL_pusherror(L, errno, "stat", "0$#");
  }
...
static int unsafe_strnlen(lua_State *L) {
  unixL_pushsize(
      L, strnlen(unixL_checklightuserdata(L, 1), unixL_checksize(L, 2)));
  return 1;
} /* unsafe_strnlen() */
...
static int tm_yday(const struct tm *tm) {
  static const int past[12] = {0,   31,  59,  90,  120, 151,
                               181, 212, 243, 273, 304, 334};
  int yday;

  if (tm->tm_yday) return tm->tm_yday;
...
static int unix_uname(lua_State *L) {
  ...
  if (-1 == uname(&name)) return unixL_pusherror(L, errno, "uname", "~$#");
  ...
  if (lua_isnoneornil(L, 1)) {
    ...
    static const char *opts[] = {"sysname", "nodename", "release",
...
static int unix_unlink(lua_State *L) {
  ...
  if (0 != unlink(path)) return unixL_pusherror(L, errno, "unlink", "0$#");
...
static int unix_unlockpt(lua_State *L) {
  ...
  if (0 != unlockpt(fd)) return unixL_pusherror(L, errno, "unlockpt", "~$#");
...
static const struct unix_const const_so[] = {
    UNIX_CONST(SO_ACCEPTCONN), UNIX_CONST(SO_BROADCAST),
    UNIX_CONST(SO_DEBUG),      UNIX_CONST(SO_DONTROUTE),
    UNIX_CONST(SO_ERROR),      UNIX_CONST(SO_KEEPALIVE),
    UNIX_CONST(SO_LINGER),
#if defined SO_NOSIGPIPE
    UNIX_CONST(SO_NOSIGPIPE),
#endif
    UNIX_CONST(SO_OOBINLINE),  UNIX_CONST(SO_RCVBUF),
    UNIX_CONST(SO_RCVLOWAT),   UNIX_CONST(SO_RCVTIMEO),
    UNIX_CONST(SO_REUSEADDR),  UNIX_CONST(SO_SNDBUF),
    UNIX_CONST(SO_SNDLOWAT),   UNIX_CONST(SO_SNDTIMEO),
    UNIX_CONST(SO_TYPE),
...
static const struct unix_const const_ipproto[] = {
    UNIX_CONST(IPPROTO_IP),  UNIX_CONST(IPPROTO_IPV6), UNIX_CONST(IPPROTO_TCP),
    UNIX_CONST(IPPROTO_UDP), UNIX_CONST(IPPROTO_ICMP),
...
static const struct unix_const const_eai[] = {
    ...
    UNIX_CONST(EAI_FAMILY),   UNIX_CONST(EAI_MEMORY),   UNIX_CONST(EAI_NONAME),
    UNIX_CONST(EAI_SERVICE),  UNIX_CONST(EAI_SOCKTYPE), UNIX_CONST(EAI_SYSTEM),
...
static const struct unix_const const_mman[] = {
    ...
    UNIX_CONST(MAP_FILE),
#endif
    UNIX_CONST(MAP_FIXED),     UNIX_CONST(MAP_PRIVATE), UNIX_CONST(MAP_SHARED),

    UNIX_CONST(MCL_CURRENT),   UNIX_CONST(MCL_FUTURE),

    UNIX_CONST(PROT_EXEC),     UNIX_CONST(PROT_NONE),   UNIX_CONST(PROT_READ),
...
static const struct unix_const const_ni[] = {
    UNIX_CONST(NI_NOFQDN),       UNIX_CONST(NI_NUMERICHOST),
    UNIX_CONST(NI_NAMEREQD),     UNIX_CONST(NI_NUMERICSERV),
...
static const struct unix_const const_poll[] = {
    UNIX_CONST(POLLERR),    UNIX_CONST(POLLHUP),    UNIX_CONST(POLLIN),
    UNIX_CONST(POLLOUT),    UNIX_CONST(POLLNVAL),   UNIX_CONST(POLLPRI),
    UNIX_CONST(POLLRDBAND), UNIX_CONST(POLLRDNORM), UNIX_CONST(POLLWRBAND),
...
static const struct unix_const const_regex[] = {
    UNIX_CONST(REG_EXTENDED), UNIX_CONST(REG_ICASE),   UNIX_CONST(REG_NOSUB),
    UNIX_CONST(REG_NEWLINE),

    UNIX_CONST(REG_NOTBOL),   UNIX_CONST(REG_NOTEOL),

    UNIX_CONST(REG_BADBR),    UNIX_CONST(REG_BADPAT),  UNIX_CONST(REG_BADRPT),
    UNIX_CONST(REG_EBRACE),   UNIX_CONST(REG_EBRACK),  UNIX_CONST(REG_ECOLLATE),
    UNIX_CONST(REG_ECTYPE),   UNIX_CONST(REG_EESCAPE), UNIX_CONST(REG_EPAREN),
    UNIX_CONST(REG_ERANGE),   UNIX_CONST(REG_ESPACE),  UNIX_CONST(REG_ESUBREG),
...
static const struct unix_const const_signal[] = {
    UNIX_CONST(SIGABRT),      UNIX_CONST(SIGALRM),
    UNIX_CONST(SIGBUS),       UNIX_CONST(SIGCHLD),
    UNIX_CONST(SIGCONT),      UNIX_CONST(SIGFPE),
    UNIX_CONST(SIGHUP),       UNIX_CONST(SIGILL),
    UNIX_CONST(SIGINT),       UNIX_CONST(SIGKILL),
    UNIX_CONST(SIGPIPE),      UNIX_CONST(SIGQUIT),
    UNIX_CONST(SIGSEGV),      UNIX_CONST(SIGSTOP),
    UNIX_CONST(SIGTERM),      UNIX_CONST(SIGTSTP),
    UNIX_CONST(SIGTTIN),      UNIX_CONST(SIGTTOU),
    UNIX_CONST(SIGUSR1),      UNIX_CONST(SIGUSR2),
    UNIX_CONST(SIGTRAP),      UNIX_CONST(SIGURG),
    UNIX_CONST(SIGXCPU),      UNIX_CONST(SIGXFSZ),
    UNIX_CONST(NSIG),

    UNIX_CONST(SIG_BLOCK),    UNIX_CONST(SIG_UNBLOCK),
    UNIX_CONST(SIG_SETMASK),

    UNIX_CONST(SA_NOCLDSTOP), UNIX_CONST(SA_ONSTACK),
    UNIX_CONST(SA_RESETHAND), UNIX_CONST(SA_RESTART),
...
static const struct unix_const const_unistd[] = {
    UNIX_CONST(STDIN_FILENO),
    UNIX_CONST(STDOUT_FILENO),
    ...
    UNIX_CONST(_SC_PAGE_SIZE),
    UNIX_CONST(_SC_PAGESIZE),
...
LUALIB_API int luaopen_unix(lua_State *L) {
   ...
   */
  lua_pushvalue(L, -1);
  unixL_newmetatable(L, "sighandler_t*", sighandler_methods,
                     sighandler_metamethods, 1);
  lua_pop(L, 1);
  ...
  for (i = 0; i < countof(unix_const); i++) {
    ...
    for (j = 0; j < size; j++) {
      ...
      if (*table[j].name >= '0' && *table[j].name <= '9')
        return luaL_error(
            L, "%s: bogus constant identifier string conversion (near %s)",
            table[j].name, (j == 0) ? "?" : table[j - 1].name);
  ...
  for (i = 0; i < countof(unix_sighandler); i++) {
    ...
    *(u_sighandler_t **)lua_newuserdata(L, sizeof unix_sighandler[i].func) =
        unix_sighandler[i].func;
    luaL_setmetatable(L, "sighandler_t*");
...
static const luaL_Reg winlib[] = {{"MessageBox", lua_MessageBox},
...
#define CHUNK_SIZE \
  16 * 1024  // 16k for one chunk,from 1k to 16k,the larger the faster(my test)
#define BOOL int
#define TRUE 1
#define FALSE 0
...
void *free_alloc(void *ud, void *ptr, size_t osize, size_t nsize) {
  if (mode == 0) {
    return pool_alloc(ud, ptr, osize, nsize);
  } else {
    return simple_alloc(ud, ptr, osize, nsize);
...
LUALIB_API int luaopen_core(lua_State *L) {
  ...
  const luaL_Reg LOGAPI[] = {{"open", lsyslog_open},
                             {"close", lsyslog_close},
                             {"push", lsyslog_log},
                             {NULL, NULL}};
...
#elif (defined _WIN32 || defined _WIN64 || defined __CYGWIN__ || \
       defined __MINGW32__ || defined LUA_USE_WINDOWS ||         \
       defined LUA_USE_MINGW)
  ...
  luaL_Reg const funcs[] = {{"enable", alloc_enable},
...
LUAMOD_API int luaopen_alloc(lua_State *L) {
  ...
  if (lua_isnil(L, -1)) {
    ...
    luaL_Reg const metamethods[] = {{"__gc", alloc_gc}, {NULL, 0}};
