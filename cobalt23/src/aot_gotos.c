...
static int jump_target(Proto *f, int pc) {
  Instruction instr = f->code[pc];
  if (GET_OPCODE(instr) != OP_JMP) {
    fatal_error("instruction is not a jump");
  }
  return (pc + 1) + GETARG_sJ(instr);
}

static void println_goto_ret() {
...
static void create_function(Proto *f) {
  ...
  for (int pc = 0; pc < f->sizecode; pc++) {
    ...
    println("  #undef  AOT_PC");
    println("  #define AOT_PC (code + %d)", pc + 1);

    int next = pc + 1;
    println("  #undef  AOT_NEXT_JUMP");
    if (next < f->sizecode && GET_OPCODE(f->code[next]) == OP_JMP) {
      println("  #define AOT_NEXT_JUMP label_%02d", jump_target(f, next));
    }

    int skip1 = pc + 2;
    println("  #undef  AOT_SKIP1");
    if (skip1 < f->sizecode) {
      println("  #define AOT_SKIP1 label_%02d", skip1);
    }
    ...
    switch (op) {
      ...
      case OP_SETTABLE: {
        ...
        println(
            "        ? (cast_void(n = ivalue(rb)), luaV_fastgeti(L, s2v(ra), "
            "n, slot))");
      ...
      case OP_NEWTABLE: {
        ...
        println("    lua_assert((!TESTARG_k(i)) == (GETARG_Ax(0x%08x) == 0));",
                f->code[pc + 1]);
      ...
      case OP_MMBIN: {
        println("    Instruction pi = 0x%08x; /* original arith. expression */",
                f->code[pc - 1]);
        println("    TValue *rb = vRB(i);");
      ...
      case OP_MMBINI: {
        println("    Instruction pi = 0x%0x;  /* original arith. expression */",
                f->code[pc - 1]);
      ...
      case OP_TAILCALL: {
        ...
        println("    if (TESTARG_k(i)) {");
        println(
            "      luaF_closeupval(L, base);  /* close upvalues from current "
            "call */");
        println(
      ...
      case OP_FORLOOP: {
        ...
        println("        goto label_%02d; /* jump back */",
                ((pc + 1) - GETARG_Bx(instr)));  //(!)
        println("      }");
        println("    }");
      ...
      case OP_FORPREP: {
        ...
        println("      goto label_%02d; /* skip the loop */",
                ((pc + 1) + GETARG_Bx(instr) + 1));  //(!)
        break;
      }
      case OP_TFORPREP: {
