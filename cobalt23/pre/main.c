...
int main(int argc, char *argv[]) {
    ...
    char command[256];
    // Make a new string, formatted_args, to hold the arguments beyond the first combined into a single string
    ...
    for (int i = 2; i < argc; i++) {
        ...
        formatted_args = realloc(formatted_args, strlen(formatted_args) + strlen(formatted_arg) + 2);
    ...
    // Format the command
    snprintf(command, sizeof(command), "cobalt -e 'import(\"preprocess\")(\"%s\", \"file\", true, %s)'", argv[1], formatted_args);
    printf("%s\n", command);
