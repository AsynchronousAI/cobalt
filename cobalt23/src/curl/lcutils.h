...
#define LCURL_CURL_VER_GE(MIN, MAJ, PAT) \
  (LIBCURL_VERSION_NUM >= LCURL_MAKE_VERSION(MIN, MAJ, PAT))

#define LCURL_CONCAT_STATIC_ASSERT_IMPL_(x, y) \
  LCURL_CONCAT1_STATIC_ASSERT_IMPL_(x, y)
#define LCURL_CONCAT1_STATIC_ASSERT_IMPL_(x, y) LCURL_UNUSED_TYPEDEF x##y
#define LCURL_STATIC_ASSERT(expr)                                              \
  typedef char LCURL_CONCAT_STATIC_ASSERT_IMPL_(static_assert_failed_at_line_, \
                                                __LINE__)[(expr) ? 1 : -1]

#define LCURL_ASSERT_SAME_SIZE(a, b) LCURL_STATIC_ASSERT(sizeof(a) == sizeof(b))
#define LCURL_ASSERT_SAME_OFFSET(a, am, b, bm) \
  LCURL_STATIC_ASSERT((offsetof(a, am)) == (offsetof(b, bm)))
#define LCURL_ASSERT_SAME_FIELD_SIZE(a, am, b, bm) \
  LCURL_ASSERT_SAME_SIZE(((a *)0)->am, ((b *)0)->bm)
...
int lcurl_storage_preserve_slist(lua_State *L, int storage,
                                 struct curl_slist *list);

struct curl_slist *lcurl_storage_remove_slist(lua_State *L, int storage,
                                              int idx);
...
int lcurl_set_callback(lua_State *L, lcurl_callback_t *c, int i,
                       const char *method);
...
int lcurl_util_pcall_method(lua_State *L, const char *name, int nargs,
