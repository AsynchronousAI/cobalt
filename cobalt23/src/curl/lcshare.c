...
int lcurl_share_create(lua_State *L, int error_mode) {
  ...
  if (!p->curl)
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_SHARE, CURLSHE_NOMEM);

  if (lua_type(L, 1) == LUA_TTABLE) {
    int ret = lcurl_utils_apply_options(L, 1, 2, 1, p->err_mode,
                                        LCURL_ERROR_SHARE, CURLSHE_BAD_OPTION);
    if (ret) return ret;
...
#define LCURL_LNG_OPT(N, S)                       \
  static int lcurl_share_set_##N(lua_State *L) {  \
...
static int lcurl_share_setopt(lua_State *L) {
  ...
  if (lua_type(L, 2) == LUA_TTABLE) {
    int ret = lcurl_utils_apply_options(L, 2, 1, 0, p->err_mode,
                                        LCURL_ERROR_SHARE, CURLSHE_BAD_OPTION);
    if (ret) return ret;
    lua_settop(L, 1);
    return 1;
