...
// fast case. We have to replace this with `goto AOT_SKIP1`
//
...
#undef  donextjump
#define donextjump(ci)	{ updatetrap(ci); goto AOT_NEXT_JUMP; }

#undef  docondjump
#define docondjump()	if (cond != GETARG_k(i)) goto AOT_SKIP1; else donextjump(ci);
...
#undef  savepc
#define savepc(L)	(ci->u.l.savedpc = AOT_PC)
...
#define aot_vmfetch(instr)	{ \
  if (l_unlikely(trap)) {  /* stack reallocation or hooks? */ \
    trap = luaG_traceexec(L, AOT_PC - 1);  /* handle hooks */ \
