...
Compiler::Compiler(lua_State* L, Proto* proto)
    : cs_(L, proto), stack_(cs_), engine_(nullptr) {
...
bool Compiler::Compile() {
  return CompileInstructions() && VerifyModule() && OptimizeModule() &&
         CreateEngine();
}

const std::string& Compiler::GetErrorMessage() { return error_; }

Engine* Compiler::GetEngine() { return engine_.release(); }
  ...
  for (cs_.curr_ = 0; cs_.curr_ < cs_.proto_->sizecode; ++cs_.curr_) {
    ...
    switch (GET_OPCODE(cs_.instr_)) {
      ...
      case OP_JMP:
        ...
        break;
      case OP_EQ:
        CompileCmp("luaV_equalobj");
        break;
      case OP_LT:
        CompileCmp("luaV_lessthan");
        break;
      case OP_LE:
        CompileCmp("luaV_lessequal");
        break;
      case OP_TEST:
        CompileTest();
        break;
      ...
      case OP_CLOSURE:
        ...
        break;
      case OP_VARARG:
        Vararg(cs_, stack_).Compile();
        break;
      case OP_EXTRAARG: /* ignored */
        break;
    }
...
void Compiler::CompileLoadbool() {
  ...
  if (GETARG_C(cs_.instr_)) cs_.B_.CreateBr(cs_.blocks_[cs_.curr_ + 2]);
}
...
void Compiler::CompileNewtable() {
  ...
  if (b != 0 || c != 0) {
    args = {cs_.values_.state, table, cs_.MakeInt(luaO_fb2int(b)),
...
void Compiler::CompileUnm() {
  ...
  auto tm_args = {cs_.values_.state, rb.GetTValue(), rb.GetTValue(),
                  ra.GetTValue(), cs_.MakeInt(TM_UNM)};
...
void Compiler::CompileBNot() {
  ...
  cs_.B_.SetInsertPoint(convert);
  auto args = {rb.GetTValue(), cs_.values_.meminteger,
  ...
  cs_.B_.SetInsertPoint(tmop);
  auto tm_args = {cs_.values_.state, rb.GetTValue(), rb.GetTValue(),
                  ra.GetTValue(), cs_.MakeInt(TM_BNOT)};
  cs_.CreateCall("luaT_trybinTM", tm_args);
  stack_.Update();
  cs_.B_.CreateBr(exit);
}
...
void Compiler::CompileTforcall() {
  ...
  cs_.SetTop(cb + 3);
  auto args = {cs_.values_.state, rcb.GetTValue(),
...
void Compiler::CompileSetlist() {
  ...
  int b = GETARG_B(cs_.instr_);
  int c = GETARG_C(cs_.instr_);
  if (c == 0) c = GETARG_Ax(cs_.proto_->code[cs_.curr_ + 1]);
...
void Compiler::CompileClosure() {
  int a = GETARG_A(cs_.instr_);
  auto& ra = stack_.GetR(a);
  auto args = {cs_.values_.state, cs_.values_.closure, cs_.GetBase(),
               ra.GetTValue(), cs_.MakeInt(GETARG_Bx(cs_.instr_))};
