...
static int l_match(lua_State* L) {
  ...
  lua_pushinteger(L, matchlength);
  return 2;
}

static int l_matchp(lua_State* L) {
  const char* pattern = luaL_checkstring(L, 1);
  const char* text = luaL_checkstring(L, 2);
  int matchlength = 0;
  int result = re_matchp(re_compile(pattern), text, &matchlength);
  if (result == -1) {
    lua_pushnil(L);
    return 1;
  }
...
static int l_compile(lua_State* L) {
  const char* pattern = luaL_checkstring(L, 1);
  printf("%s\n", pattern);
  re_t result = re_compile(pattern);
  if (result == NULL) {
    lua_pushinteger(L, 0);
    return 1;
  }
...
static int l_print(lua_State* L) {
  re_t pattern = lua_touserdata(L, 1);
  re_print(pattern);
  return 0;
}
...
static const luaL_Reg lregex[] = {{"match", l_match},     {"matchp", l_matchp},
                                  {"compile", l_compile}, {"print", l_print},
                                  {"free", l_free},       {NULL, NULL}};

LUAMOD_API int luaopen_regex(lua_State* L) {
  luaL_newlib(L, lregex);
  return 1;
}