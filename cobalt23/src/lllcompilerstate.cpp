...
llvm::Function* CompilerState::CreateMainFunction() {
  ...
  std::vector<llvm::Type*> params = {rt_.GetType("lua_State"),
...
llvm::Value* CompilerState::MakeInt(int64_t value, llvm::Type* type) {
  if (!type) type = rt_.MakeIntT(sizeof(int));
...
llvm::Value* CompilerState::GetFieldPtr(llvm::Value* strukt,
                                        llvm::Type* fieldtype, size_t offset,
...
llvm::Value* CompilerState::CreateCall(
    const std::string& name, std::initializer_list<llvm::Value*> argslist,
...
llvm::Value* CompilerState::GetBase() { return B_.CreateLoad(values_.base); }
...
llvm::BasicBlock* CompilerState::CreateSubBlock(const std::string& suffix,
  ...
  if (!preview) preview = blocks_[curr_];
