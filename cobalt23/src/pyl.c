...
static int LuaObject_setattr(PyObject *obj, PyObject *attr, PyObject *value)
{
    ...
    if (!lua_istable(LuaState, -1)) {
        lua_pop(LuaState, -1);
        PyErr_SetString(PyExc_TypeError, "Cobalt object is not a table");
        return -1;
    }
...
static PyObject *LuaObject_str(PyObject *obj)
{
    ...
    if (!ret) {
        ...
        switch (type) {
            case LUA_TTABLE:
            case LUA_TFUNCTION:
                ret = PyUnicode_FromFormat("<Cobalt %s at %p>",
            ...
            default:
                ret = PyUnicode_FromFormat("<Cobalt %s>",
                    lua_typename(LuaState, type));
                break;
...
static PyMethodDef lua_methods[] =
{
    ...
    {"import",     Lua_require,    METH_VARARGS,        NULL},
    {NULL,         NULL}
};
...
static struct PyModuleDef lua_module =
{
    ...
    "cobalt",
    "Python bridge to the Cobalt Programming Language",
    -1,
    lua_methods
};
#endif
...
PyMODINIT_FUNC PyInit_lua(void)
{
  ...
  if (PyType_Ready(&LuaObject_Type) < 0 ||
      ...
      return NULL;
#else
      (m = Py_InitModule3("cobalt", lua_methods,
                          "Python bridge to the Cobalt Programming Language")) == NULL)
      return;
#endif
