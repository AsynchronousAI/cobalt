...
int freeuserdata(lua_State *L, ud_t *ud, const char *tracename) {
  /* The 'Valid' mark prevents double calls when an object is explicitly
   * destroyed, and subsequently deleted also by the GC (the ud sticks around
   * until the GC collects it, so we mark it as invalid when the object is
   * explicitly destroyed by the script, or implicitly destroyed because child
   * of a destroyed object). */
  if (!IsValid(ud)) return 0;
  CancelValid(ud);
  if (ud->info) Free(L, ud->info);
...
static int freeifchild(lua_State *L, const void *mem, const char *mt,
                       const void *parent_ud)
/* callback for udata_scan */
{
  ud_t *ud = (ud_t *)mem;
  (void)mt;
  if (IsValid(ud) && (ud->parent_ud == parent_ud)) ud->destructor(L, ud);
  return 0;
}

int freechildren(lua_State *L, const char *mt, ud_t *parent_ud)
/* calls the self destructor for all 'mt' objects that are children of the given
   parent_ud */
{
