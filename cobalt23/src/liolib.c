...
*/
int isbdigit(int c) { return (c == '0' || c == '1'); }
int isodigit(int c) { return (c >= '0' && c <= '7'); }
static int readdigits(RN *rn, int hex, int bin, int oct) {
  int count = 0;
  while ((hex ? isxdigit(rn->c)
              : (bin ? isbdigit(rn->c)
                     : (oct ? isodigit(rn->c) : isdigit(rn->c)))) &&
         nextc(rn))
    count++;
  return count;
}
...
static int read_number(lua_State *L, FILE *f) {
  ...
  if (test2(&rn, "00")) {
    ...
    if (test2(&rn, "bB"))
      bin = 1; /* numeral is binary */
    else
      count = 1; /* count initial '0' as a valid digit */

    if (test2(&rn, "oO"))
      oct = 1; /* numeral is ocatal */
    else
      count = 1; /* count initial '0' as a valid digit */
  }
  ...
  if (count > 0 &&
      test2(&rn, ((hex | bin | oct) ? "pP" : "eE"))) { /* exponent mark? */
