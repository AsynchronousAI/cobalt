...
static int Update(lua_State *L) {
  ...
  ec = SDL_UpdateTexture(texture, (ec == ERR_NOTPRESENT) ? NULL : &rect, pixels,
                         pitch);
  CheckError(L, ec);
  return 0;
}
...
static int Lock(lua_State *L) {
  ...
  texture_t *texture = checktexture(L, 1, NULL);
  int ec = optrect(L, 2, &rect);
  ec = SDL_LockTexture(texture, (ec == ERR_NOTPRESENT) ? NULL : &rect, &pixels,
                       &pitch);
  CheckError(L, ec);
...
static int LockToSurface(lua_State *L) {
  ...
  texture_t *texture = checktexture(L, 1, NULL);
  int ec = optrect(L, 2, &rect);
  ec = SDL_LockTextureToSurface(texture, (ec == ERR_NOTPRESENT) ? NULL : &rect,
                                &surface);
  if (UD(surface))
...
static int ComposeCustomBlendMode(lua_State *L) {
  ...
  SDL_BlendMode mode = SDL_ComposeCustomBlendMode(
      srcColorFactor, dstColorFactor, colorOperation, srcAlphaFactor,
