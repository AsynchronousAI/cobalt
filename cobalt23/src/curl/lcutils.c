...
int lcurl_set_callback(lua_State *L, lcurl_callback_t *c, int i,
  ...
  if (lutil_is_null(L, i)) {
    if (top == (i + 1)) {
      ...
      luaL_argcheck(L, lua_isnoneornil(L, i + 1) || lutil_is_null(L, i + 1),
...
int lcurl_utils_apply_options(lua_State *L, int opt, int obj, int do_close,
  ...
  while (lua_next(L, opt) != 0) {
    ...
    if (lua_type(L, -2) ==
        LUA_TNUMBER) { /* [curl.OPT_URL] = "http://localhost" */
      lua_pushvalue(L, -2);
...
curl_socket_t lcurl_opt_os_socket(lua_State *L, int idx, curl_socket_t def) {
  if (lua_islightuserdata(L, idx)) return (curl_socket_t)lua_touserdata(L, idx);
...
void lcurl_push_os_socket(lua_State *L, curl_socket_t fd) {
  ...
  lutil_pushint64(L, fd);
  if (lcurl_opt_os_socket(L, -1, 0) != fd) lua_pushlightuserdata(L, (void *)fd);
