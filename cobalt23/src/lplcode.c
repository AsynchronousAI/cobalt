...
tailcall:
  switch (tree->tag) {
    ...
    case TUTFR: {
      ...
      for (c = tree->key; c <= sib1(tree)->key; c++) setchar(firstset->cs, c);
      return 0;
    }
    case TTrue: {
...
static void codethrow(CompileState *compst, TTree *throw) {
  ...
  getinstr(compst, aux).i.key =
      throw->key;                    /* next instruction keeps only rule name */
...
*/
static void correctcalls(CompileState *compst, int *positions, int from,
                         int to) {
  int i;
...
*/
static int codeseq1(CompileState *compst, TTree *p1, TTree *p2, int tt,
                    const Charset *fl) {
  if (needfollow(p1)) {
    Charset fl1;
