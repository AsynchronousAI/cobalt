...
static int str_find_aux (lua_State *L, int find) {
  ...
  lua_Integer init = posrelat(luaL_optinteger(L, 3, LUA_INDEX_BASE), ls);
  if (init < 1) init = 1;
  ...
  if (find && (lua_toboolean(L, 4) || nospecials(p, lp))) {
    ...
    if (s2) {
      lua_pushinteger(L, (s2 - s) + LUA_INDEX_BASE);
    ...
    do {
      ...
      if ((res=match(&ms, s1, p)) != NULL) {
        if (find) {
          lua_pushinteger(L, (s1 - s) + LUA_INDEX_BASE);  /* start */
...
static int str_replace (lua_State *L) {
  ...
  const char *p2 = luaL_checklstring(L, 3, &lp2);  /* pattern */
  lua_Integer max_s = luaL_optinteger(L, 4, srcl + 1);  /* max replacements */

  luaL_Buffer b;
  luaL_buffinit(L, &b);
  lua_Integer n = 0, init = 0;  /* replacement count */
