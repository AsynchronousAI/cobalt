...
#include "call_x64.h"
    ...
    defined ARM || defined _ARM_ || defined ARMV4I || defined _M_ARM
...
#define LINKTABLE_MAX_SIZE \
  (sizeof(extnames) / sizeof(extnames[0]) * (JUMP_SIZE))

static cfunction compile(struct jit* jit, lua_State* L, cfunction func,
                         int ref) {
...
static void* reserve_code(struct jit* jit, lua_State* L, size_t sz) {
  ...
  if (off + sz >= size) {
    ...
    jit->pages = (struct page**)realloc(
        jit->pages, (++jit->pagenum) * sizeof(jit->pages[0]));

    size = ALIGN_UP(sz + LINKTABLE_MAX_SIZE + sizeof(struct page),
                    jit->align_page_size);
