...
static int type_error(lua_State* L, int idx, const char* to_type, int to_usr,
  ...
  if (ft.type != INVALID_TYPE) {
    ...
    lua_pushfstring(L, "unable to convert argument %d from cdata<%s> to cdata<",
                    idx, lua_tostring(L, -1));
    lua_remove(L, -2);
    luaL_addvalue(&B);
  } else {
    lua_pushfstring(L, "unable to convert argument %d from lua<%s> to cdata<",
...
static int64_t check_intptr(lua_State* L, int idx, void* p, struct ctype* ct) {
  ...
  switch (ct->type) {
    ...
    case INT32_TYPE:
      return ct->is_unsigned ? (int64_t) * (uint32_t*)p
                             : (int64_t) * (int32_t*)p;

    case INT16_TYPE:
      return ct->is_unsigned ? (int64_t) * (uint16_t*)p
                             : (int64_t) * (int16_t*)p;
...
#define TO_NUMBER(TYPE, ALLOW_POINTERS, LUA_TONUMBER)     \
  ...
  switch (lua_type(L, idx)) {                             \
    ...
    case LUA_TUSERDATA:                                   \
      ...
      } else if (ct.pointers || ct.type == STRUCT_TYPE || \
                 ct.type == UNION_TYPE) {                 \
        if (!ALLOW_POINTERS) {                            \
          type_error(L, idx, #TYPE, 0, NULL);             \
        }                                                 \
...
static size_t unpack_vararg(lua_State* L, int i, char* to) {
  ...
  switch (lua_type(L, i)) {
    ...
    case LUA_TNUMBER:
      *(double*)to = lua_tonumber(
          L, i);  // TODO in Lua 5.3: lua_tointeger sometimes should be here
...
void unpack_varargs_stack_skip(lua_State* L, int first, int last,
                               int ints_to_skip, int floats_to_skip, char* to) {
  int i;
...
void unpack_varargs_float(lua_State* L, int first, int last, int max,
                          char* to) {
  int i;
 ...
 */
int32_t check_enum(lua_State* L, int idx, int to_usr,
                   const struct ctype* to_ct) {
  int32_t ret;
...
static int is_same_type(lua_State* L, int usr1, int usr2,
                        const struct ctype* t1, const struct ctype* t2) {
  if (t1->type != t2->type) {
    return 0;
  }
 ...
 * structs on the fly. */
void* check_typed_pointer(lua_State* L, int idx, int to_usr,
  ...
  if (tt->pointers == 1 &&
      (tt->type == STRUCT_TYPE || tt->type == UNION_TYPE) &&
...
static void set_value(lua_State* L, int idx, void* to, int to_usr,
                      const struct ctype* tt, int check_pointers);

static void set_array(lua_State* L, int idx, void* to, int to_usr,
                      const struct ctype* tt, int check_pointers) {
  size_t i, sz, esz;
  struct ctype et;
  ...
  switch (lua_type(L, idx)) {
    case LUA_TTABLE:
      ...
      while (lua_next(L, idx)) {
        ...
        if (!have_first && lua_tonumber(L, -2) == 1 &&
            lua_tonumber(L, -1) != 0) {
          have_first = 1;
        } else if (!have_other && (lua_type(L, -2) != LUA_TNUMBER ||
                                   lua_tonumber(L, -2) != 1)) {
          have_other = 1;
        }
      ...
      /* if we only had a single non zero value then initialize all members to
       * that value */
...
static void set_value(lua_State* L, int idx, void* to, int to_usr,
  ...
  if (tt->is_array) {
    ...
    if (lua_istable(L, idx)) {
      luaL_error(
          L, "Can't set a pointer member to a struct that's about to be freed");
    }
    ...
    uint64_t hi_mask =
        UINT64_C(0) - (UINT64_C(1) << (tt->bit_offset + tt->bit_size));
 ...
 */
static ptrdiff_t lookup_cdata_index(lua_State* L, int idx, int ct_usr,
                                    struct ctype* ct) {
  struct ctype mt;
  ptrdiff_t off;
...
static int cdata_index(lua_State* L) {
  ...
  if (!ct.pointers) {
    switch (ct.type) {
      case FUNCTION_PTR_TYPE:
        /* Callbacks use the same metatable as standard cdata values, but have
         * set and free members. So instead of mt.__index = mt, we do the equiv
         * here. */
...
static complex_double check_complex(lua_State* L, int idx, void* p,
                                    struct ctype* ct) {
  if (ct->type == INVALID_TYPE) {
...
static void push_number(lua_State* L, int64_t val, int ct_usr,
                        const struct ctype* ct) {
 ...
 * arguments */
static int call_user_binop(lua_State* L, const char* opfield, int lidx,
                           int lusr, const struct ctype* lt, int ridx, int rusr,
...
static int cdata_add(lua_State* L) {
  ...
  if (IS_COMPLEX(ct.type)) {
    ...
    /* note due to 2s complement it doesn't matter if we do the addition as int
     * or uint, but the result needs to be uint64_t if either of the sources are
     */
    ...
    if (lt.pointers && rt.pointers) {
      ...
      int64_t res =
          left + (lt.pointers > 1 ? sizeof(void*) : lt.base_size) * right;
      lt.is_array = 0;
      push_number(L, res, 3, &lt);

    } else if (rt.pointers) {
      int64_t res =
          right + (rt.pointers > 1 ? sizeof(void*) : rt.base_size) * left;
      rt.is_array = 0;
...
#else
#define MULC(l, r, s)                         \
  s.real = l.real * r.real - l.imag * r.imag, \
  s.imag = l.real * r.imag + l.imag * r.real
#define DIVC(l, r, s)                            \
  s.real = (l.real * r.real + l.imag * r.imag) / \
           (r.real * r.real + r.imag * r.imag),  \
  s.imag = (l.imag * r.real - l.real * r.imag) / \
           (r.real * r.real + r.imag * r.imag)
#define MODC(l, r, s) \
  (void)l, (void)r, memset(&s, 0, sizeof(s)), luaL_error(L, "NYI: complex mod")
#define POWC(l, r, s) \
...
static void print_type(lua_State* L, const struct ctype* ct) {
  lua_pushfstring(
      L,
      " sz %d %d %d align %d ptr %d %d %d type %s%s %d %d %d name %d call %d "
      "%d var %d %d %d bit %d %d %d %d jit %d",
      /* sz */
...
static int cdata_tostring(lua_State* L) {
  ...
  if (ct.pointers > 0 || ct.is_reference || ct.type == STRUCT_TYPE ||
      ct.type == UNION_TYPE) {
    push_type_name(L, -1, &ct);
...
static void push_builtin(lua_State* L, struct ctype* ct, const char* name,
                         int type, int size, int align, int is_unsigned) {
  memset(ct, 0, sizeof(*ct));
  ct->type = type;
  ct->base_size = size;
  ct->align_mask = align;
...
static void push_builtin_undef(lua_State* L, struct ctype* ct, const char* name,
                               int type) {
  memset(ct, 0, sizeof(*ct));
  ct->type = type;
...
#undef STR2
...
#else
    GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (char*)&_fmode,
                       &libs[2]);
  ...
  /* setup builtin types */
  {
    ...
    push_builtin(L, &ct, "uint16_t", INT16_TYPE, sizeof(uint16_t), ALIGNOF(a16),
                 1);
    push_builtin(L, &ct, "int16_t", INT16_TYPE, sizeof(int16_t), ALIGNOF(a16),
                 0);
    push_builtin(L, &ct, "uint32_t", INT32_TYPE, sizeof(uint32_t), ALIGNOF(a32),
                 1);
    push_builtin(L, &ct, "int32_t", INT32_TYPE, sizeof(int32_t), ALIGNOF(a32),
                 0);
    push_builtin(L, &ct, "uint64_t", INT64_TYPE, sizeof(uint64_t), ALIGNOF(a64),
                 1);
...
#endif
    push_builtin(L, &ct, "uintptr_t", INTPTR_TYPE, sizeof(uintptr_t),
                 ALIGNOF(aptr), 1);
    push_builtin(L, &ct, "intptr_t", INTPTR_TYPE, sizeof(uintptr_t),
                 ALIGNOF(aptr), 0);
    push_builtin(L, &ct, "complex float", COMPLEX_FLOAT_TYPE,
                 sizeof(complex_float), ALIGNOF(cf), 0);
