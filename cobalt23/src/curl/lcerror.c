...
static const char *_lcurl_err_mnemo(int tp, int err) {
  switch (tp) {
    case LCURL_ERROR_EASY:
      return lcurl_err_easy_mnemo(err);
    case LCURL_ERROR_MULTI:
      return lcurl_err_multi_mnemo(err);
    case LCURL_ERROR_SHARE:
      return lcurl_err_share_mnemo(err);
    case LCURL_ERROR_FORM:
      return lcurl_err_form_mnemo(err);
    case LCURL_ERROR_URL:
      return lcurl_err_url_mnemo(err);
  }
  assert(0);
...
static const char *_lcurl_err_msg(int tp, int err) {
  switch (tp) {
    ...
    case LCURL_ERROR_SHARE:
      return curl_share_strerror(err);
    case LCURL_ERROR_FORM:
      return lcurl_err_form_mnemo(err);
    case LCURL_ERROR_URL:
      return lcurl_err_url_mnemo(err);
...
static const char *_lcurl_err_category_name(int tp) {
  assert((tp == LCURL_ERROR_EASY) || (tp == LCURL_ERROR_MULTI) ||
         (tp == LCURL_ERROR_SHARE) || (tp == LCURL_ERROR_FORM) ||
         (tp == LCURL_ERROR_URL) || 0);
...
static void _lcurl_err_pushstring(lua_State *L, int tp, int err) {
  lua_pushfstring(L, "[%s][%s] %s (%d)", _lcurl_err_category_name(tp),
                  _lcurl_err_mnemo(tp, err), _lcurl_err_msg(tp, err), err);
}
...
int lcurl_error_create(lua_State *L, int error_type, int no) {
  ...
  assert(
      (error_type == LCURL_ERROR_EASY) || (error_type == LCURL_ERROR_MULTI) ||
      (error_type == LCURL_ERROR_SHARE) || (error_type == LCURL_ERROR_FORM) ||
      (error_type == LCURL_ERROR_URL) || 0);
...
#if COBALTLUA_VERSION_NUM >= 502  // lua 5.2
  lcurl_error_create(L, error_type, code);
#else
...
static const int ERROR_CATEGORIES[] = {
    LCURL_ERROR_EASY, LCURL_ERROR_MULTI, LCURL_ERROR_SHARE,
    LCURL_ERROR_FORM, LCURL_ERROR_URL,
};

static const char *ERROR_CATEGORIES_NAME[] = {
    LCURL_ERROR_EASY_NAME, LCURL_ERROR_MULTI_NAME, LCURL_ERROR_SHARE_NAME,
    LCURL_ERROR_FORM_NAME, LCURL_ERROR_URL_NAME,   NULL};
...
/* libcurl rename CURLE_FTP_WEIRD_SERVER_REPLY to CURLE_WEIRD_SERVER_REPLY in
 * version 7.51.0*/
/* we can not have both codes in general because we have to be able convern
 * error number to error name*/
...
void lcurl_error_initlib(lua_State *L, int nup) {
  ...
  lua_pushstring(L, _lcurl_err_category_name(LCURL_ERROR_EASY));
  lua_setfield(L, -2, "ERROR_EASY");
  lua_pushstring(L, _lcurl_err_category_name(LCURL_ERROR_MULTI));
  lua_setfield(L, -2, "ERROR_MULTI");
  lua_pushstring(L, _lcurl_err_category_name(LCURL_ERROR_SHARE));
  lua_setfield(L, -2, "ERROR_SHARE");
  lua_pushstring(L, _lcurl_err_category_name(LCURL_ERROR_FORM));
  lua_setfield(L, -2, "ERROR_FORM");
}
