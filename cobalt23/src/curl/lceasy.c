...
int lcurl_easy_create(lua_State *L, int error_mode) {
  ...
  if (!p->curl)
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_EASY, CURLE_FAILED_INIT);
  ...
  if (lua_type(L, 1) == LUA_TTABLE) {
    int ret = lcurl_utils_apply_options(
        L, 1, 2, 1, p->err_mode, LCURL_ERROR_EASY, LCURL_E_UNKNOWN_OPTION);
    if (ret) return ret;
...
static int lcurl_easy_cleanup(lua_State *L) {
  ...
  if (p->multi) {
    LCURL_UNUSED_VAR CURLMcode code =
        lcurl__multi_remove_handle(L, p->multi, p);
  ...
  if (p->curl) {
    ...
    curL = p->L;
    lcurl__easy_assign_lua(L, p, L, 1);
...
static int lcurl_easy_perform(lua_State *L) {
  ...
  curL = p->L;
  lcurl__easy_assign_lua(L, p, L, 0);
  ...
  if ((lua_gettop(L) > top) &&
      (lua_touserdata(L, top + 1) == LCURL_ERROR_TAG)) {
    return lua_error(L);
  }
...
static int lcurl_easy_unescape(lua_State *L) {
  ...
  int ret_size;
  const char *ret =
      curl_easy_unescape(p->curl, data, (int)data_size, &ret_size);
  if (!ret) {
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_EASY, CURLE_OUT_OF_MEMORY);
  }
...
static int lcurl_opt_set_long_(lua_State *L, int opt) {
  ...
  if (lua_isboolean(L, 2)) {
    ...
    if (val && ((opt == CURLOPT_SSL_VERIFYHOST)
    ...
    luaL_argcheck(L, lua_type(L, 2) == LUA_TNUMBER, 2,
                  "number or boolean expected");
...
static int lcurl_opt_set_string_(lua_State *L, int opt, int store) {
  ...
  luaL_argcheck(L, lua_type(L, 2) == LUA_TSTRING || lutil_is_null(L, 2), 2,
                "string expected");
...
static int lcurl_opt_set_slist_(lua_State *L, int opt, int list_no) {
  ...
  luaL_argcheck(L, list || lua_istable(L, 2) || lutil_is_null(L, 2), 2,
                "array expected");
...
static size_t lcurl_hpost_read_callback(char *buffer, size_t size,
...
static int lcurl_easy_set_callback(lua_State *L, lcurl_easy_t *p,
                                   lcurl_callback_t *c, int OPT_CB, int OPT_UD,
...
static size_t lcurl_write_callback_(lua_State *L, lcurl_easy_t *p,
                                    lcurl_callback_t *c, char *ptr, size_t size,
                                    size_t nmemb) {
...
static size_t lcurl_write_callback(char *ptr, size_t size, size_t nmemb,
                                   void *arg) {
  lcurl_easy_t *p = arg;
  assert(NULL != p->L);
...
static int lcurl_easy_set_WRITEFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->wr, CURLOPT_WRITEFUNCTION,
                                 CURLOPT_WRITEDATA, "write",
                                 lcurl_write_callback);
}
...
static size_t lcurl_easy_read_callback(char *buffer, size_t size, size_t nitems,
                                       void *arg) {
  lcurl_easy_t *p = arg;
...
static int lcurl_easy_set_READFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->rd, CURLOPT_READFUNCTION,
                                 CURLOPT_READDATA, "read",
                                 lcurl_easy_read_callback);
}
...
static size_t lcurl_header_callback(char *ptr, size_t size, size_t nmemb,
                                    void *arg) {
  lcurl_easy_t *p = arg;
  assert(NULL != p->L);
...
static int lcurl_easy_set_HEADERFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->hd, CURLOPT_HEADERFUNCTION,
                                 CURLOPT_HEADERDATA, "header",
                                 lcurl_header_callback);
}
...
static int lcurl_xferinfo_callback(void *arg, curl_off_t dltotal,
...
static int lcurl_progress_callback(void *arg, double dltotal, double dlnow,
  ...
  return lcurl_xferinfo_callback(arg, (curl_off_t)dltotal, (curl_off_t)dlnow,
                                 (curl_off_t)ultotal, (curl_off_t)ulnow);
}

static int lcurl_easy_set_PROGRESSFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  int n = lcurl_easy_set_callback(L, p, &p->pr, CURLOPT_PROGRESSFUNCTION,
                                  CURLOPT_PROGRESSDATA, "progress",
                                  lcurl_progress_callback);

#if LCURL_CURL_VER_GE(7, 32, 0)
  if (p->pr.cb_ref != LUA_NOREF) {
    curl_easy_setopt(p->curl, CURLOPT_XFERINFOFUNCTION,
                     lcurl_xferinfo_callback);
...
static int lcurl_easy_set_SEEKFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->seek, CURLOPT_SEEKFUNCTION,
                                 CURLOPT_SEEKDATA, "seek", lcurl_seek_callback);
}
...
static int lcurl_debug_callback(CURL *handle, curl_infotype type, char *data,
...
static int lcurl_easy_set_DEBUGFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->debug, CURLOPT_DEBUGFUNCTION,
                                 CURLOPT_DEBUGDATA, "debug",
                                 lcurl_debug_callback);
}
...
static int lcurl_match_callback(void *arg, const char *pattern,
                                const char *string) {
  lcurl_easy_t *p = arg;
  lua_State *L = p->L;
  ...
  if (lua_gettop(L) > top) {
    ...
    ret = lua_toboolean(L, top + 1) ? CURL_FNMATCHFUNC_MATCH
...
static int lcurl_easy_set_FNMATCH_FUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->match, CURLOPT_FNMATCH_FUNCTION,
                                 CURLOPT_FNMATCH_DATA, "match",
                                 lcurl_match_callback);
}
...
static int lcurl_chunk_bgn_callback(struct curl_fileinfo *info, void *arg,
                                    int remains) {
  lcurl_easy_t *p = arg;
  lua_State *L = p->L;
  ...
  lua_pushinteger(L, info->filetype);
  lua_setfield(L, -2, "filetype");
  lutil_pushint64(L, info->time);
  lua_setfield(L, -2, "time");
  lutil_pushint64(L, info->perm);
  lua_setfield(L, -2, "perm");
  lua_pushinteger(L, info->uid);
  lua_setfield(L, -2, "uid");
  lua_pushinteger(L, info->gid);
  lua_setfield(L, -2, "gid");
  lutil_pushint64(L, info->size);
  lua_setfield(L, -2, "size");
  lutil_pushint64(L, info->hardlinks);
  lua_setfield(L, -2, "hardlinks");
  lutil_pushint64(L, info->flags);
  lua_setfield(L, -2, "flags");

  lua_newtable(L);
  if (info->strings.time) {
    lua_pushstring(L, info->strings.time);
    lua_setfield(L, -2, "time");
  }
  if (info->strings.perm) {
    lua_pushstring(L, info->strings.perm);
    lua_setfield(L, -2, "perm");
  }
  if (info->strings.user) {
    lua_pushstring(L, info->strings.user);
    lua_setfield(L, -2, "user");
  }
  if (info->strings.group) {
    lua_pushstring(L, info->strings.group);
    lua_setfield(L, -2, "group");
  }
  if (info->strings.target) {
    lua_pushstring(L, info->strings.target);
  ...
  if (lua_gettop(L) > top) {
    ...
    ret = lua_toboolean(L, top + 1) ? CURL_CHUNK_BGN_FUNC_OK
...
static int lcurl_chunk_end_callback(void *arg) {
  ...
  if (lua_gettop(L) > top) {
    ...
    ret = lua_toboolean(L, top + 1) ? CURL_CHUNK_END_FUNC_OK
...
static int lcurl_easy_set_TRAILERFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->trailer, CURLOPT_TRAILERFUNCTION,
                                 CURLOPT_TRAILERDATA, "trailer",
                                 lcurl_trailer_callback);
}
...
static int lcurl_hstsread_callback(CURL *easy, struct curl_hstsentry *sts,
                                   void *arg) {
  lcurl_easy_t *p = arg;
  lua_State *L = p->L;
  int top = lua_gettop(L);
...
static int lcurl_easy_set_HSTSREADFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->hstsread, CURLOPT_HSTSREADFUNCTION,
                                 CURLOPT_HSTSREADDATA, "hstsread",
                                 lcurl_hstsread_callback);
}
...
static int lcurl_hstswrite_callback(CURL *easy, struct curl_hstsentry *sts,
                                    struct curl_index *count, void *arg) {
  lcurl_easy_t *p = arg;
  lua_State *L = p->L;
  int top = lua_gettop(L);
...
static int lcurl_easy_set_HSTSWRITEFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->hstswrite, CURLOPT_HSTSWRITEFUNCTION,
                                 CURLOPT_HSTSWRITEDATA, "hstswrite",
                                 lcurl_hstswrite_callback);
}
...
static int lcurl_ssh_key_callback(CURL *easy, const struct curl_khkey *knownkey,
                                  const struct curl_khkey *foundkey,
                                  enum curl_khmatch khmatch, void *arg) {
  lcurl_easy_t *p = arg;
  lua_State *L = p->L;
  int top = lua_gettop(L);
...
static int lcurl_easy_set_SSH_KEYFUNCTION(lua_State *L) {
  lcurl_easy_t *p = lcurl_geteasy(L);
  return lcurl_easy_set_callback(L, p, &p->ssh_key, CURLOPT_SSH_KEYFUNCTION,
                                 CURLOPT_SSH_KEYDATA, "ssh_key",
                                 lcurl_ssh_key_callback);
}
...
static int lcurl_easy_setopt(lua_State *L) {
  ...
  if (lua_type(L, 2) == LUA_TTABLE) {
    int ret = lcurl_utils_apply_options(
        L, 2, 1, 0, p->err_mode, LCURL_ERROR_EASY, LCURL_E_UNKNOWN_OPTION);
    if (ret) return ret;
    lua_settop(L, 1);
    return 1;
  }
...
#define OPT_ENTRY(l, N, T, S, D) \
  case CURLOPT_##N:              \
    return lcurl_easy_set_##N(L);
  switch (opt) {
#include "lcopteasy.h"
...
#define OPT_ENTRY(l, N, T, S) \
  case CURLINFO_##N:          \
    return lcurl_easy_get_##N(L);
  switch (opt) {
#include "lcinfoeasy.h"
  }
#undef OPT_ENTRY
...
#include "lcopteasy.h"
    OPT_ENTRY(postfields, POSTFIELDS, TTT, 0, 0) OPT_ENTRY(
        httppost, HTTPPOST, TTT, 0,
        0) OPT_ENTRY(share, SHARE, TTT, 0,
                     0) OPT_ENTRY(writefunction, WRITEFUNCTION, TTT, 0, 0)
        OPT_ENTRY(readfunction, READFUNCTION, TTT, 0, 0) OPT_ENTRY(
            headerfunction, HEADERFUNCTION, TTT, 0,
            0) OPT_ENTRY(progressfunction, PROGRESSFUNCTION, TTT, 0,
                         0) OPT_ENTRY(seekfunction, SEEKFUNCTION, TTT, 0,
                                      0) OPT_ENTRY(debugfunction, DEBUGFUNCTION,
                                                   TTT, 0, 0)
#if LCURL_CURL_VER_GE(7, 19, 6)
            OPT_ENTRY(ssh_keyfunction, SSH_KEYFUNCTION, TTT, 0, 0)
#endif
#if LCURL_CURL_VER_GE(7, 21, 0)
                OPT_ENTRY(fnmatch_function, FNMATCH_FUNCTION, TTT, 0, 0) OPT_ENTRY(
                    chunk_bgn_function,
                    CHUNK_BGN_FUNCTION,
                    TTT, 0,
                    0) OPT_ENTRY(chunk_end_function, CHUNK_END_FUNCTION, TTT, 0,
                                 0)
#endif
#if LCURL_CURL_VER_GE(7, 46, 0)
                    OPT_ENTRY(
                        ...
                        STREAM_DEPENDS,
                        TTT, 0,
                        0) OPT_ENTRY(stream_depends_e, STREAM_DEPENDS_E, TTT, 0, 0)
#endif
#if LCURL_CURL_VER_GE(7, 56, 0)
                        OPT_ENTRY(mimepost, MIMEPOST, TTT, 0, 0)
#endif
...
#endif
#if LCURL_CURL_VER_GE(7, 64, 0)
                                OPT_ENTRY(trailerfunction, TRAILERFUNCTION, TTT,
                                          0, 0)
#endif
#if LCURL_CURL_VER_GE(7, 74, 0) && LCURL_USE_HSTS
                                    OPT_ENTRY(
                                        ...
                                        HSTSREADFUNCTION,
                                        TTT, 0,
                                        0) OPT_ENTRY(hstswritefunction,
                                                     HSTSWRITEFUNCTION, TTT, 0, 0)
#endif
#undef OPT_ENTRY
...
#include "lcopteasy.h"
                                        OPT_ENTRY(postfields, POSTFIELDS, TTT, 0, 0) OPT_ENTRY(
                                            ...
                                            HTTPPOST,
                                            TTT, 0,
                                            0) OPT_ENTRY(share, SHARE, TTT, 0, 0)
                                            OPT_ENTRY(writefunction,
                                                      WRITEFUNCTION,
                                                      TTT, 0,
                                                      0) OPT_ENTRY(readfunction,
                                                                   READFUNCTION, TTT, 0, 0)
                                                OPT_ENTRY(
                                                    headerfunction,
                                                    HEADERFUNCTION,
                                                    TTT, 0,
                                                    0) OPT_ENTRY(progressfunction,
                                                                 PROGRESSFUNCTION, TTT, 0, 0)
                                                    OPT_ENTRY(seekfunction,
                                                              SEEKFUNCTION,
                                                              TTT, 0,
                                                              0)
                                                        OPT_ENTRY(debugfunction,
                                                                  DEBUGFUNCTION,
                                                                  TTT, 0,
                                                                  0)
#if LCURL_CURL_VER_GE(7, 19, 6)
                                                            OPT_ENTRY(
                                                                ...
                                                                OPT_ENTRY(
                                                                    ...
                                                                    FNMATCH_FUNCTION,
                                                                    TTT,
                                                                    0, 0) OPT_ENTRY(chunk_bgn_function,
                                                                                    CHUNK_BGN_FUNCTION,
                                                                                    TTT,
                                                                                    0, 0)
                                                                    OPT_ENTRY(
...
#include "lcopteasy.h"
    OPT_ENTRY(postfields, POSTFIELDS, TTT, 0, 0) OPT_ENTRY(
        httppost, HTTPPOST, TTT, 0, 0) OPT_ENTRY(share, SHARE, TTT, 0, 0)
        OPT_ENTRY(writefunction, WRITEFUNCTION, TTT, 0, 0) OPT_ENTRY(
            readfunction, READFUNCTION, TTT, 0,
            0) OPT_ENTRY(headerfunction, HEADERFUNCTION, TTT, 0, 0)
            OPT_ENTRY(progressfunction, PROGRESSFUNCTION, TTT, 0, 0) OPT_ENTRY(
                seekfunction, SEEKFUNCTION, TTT, 0,
                0) OPT_ENTRY(debugfunction, DEBUGFUNCTION, TTT, 0, 0)
#if LCURL_CURL_VER_GE(7, 19, 6)
                OPT_ENTRY(ssh_keyfunction, SSH_KEYFUNCTION, TTT, 0, 0)
#endif
#if LCURL_CURL_VER_GE(7, 21, 0)
                    OPT_ENTRY(fnmatch_function, FNMATCH_FUNCTION, TTT, 0, 0)
                        OPT_ENTRY(chunk_bgn_function, CHUNK_BGN_FUNCTION, TTT,
                                  0, 0) OPT_ENTRY(chunk_end_function,
                                                  CHUNK_END_FUNCTION, TTT, 0, 0)
#endif
#if LCURL_CURL_VER_GE(7, 46, 0)
                            OPT_ENTRY(stream_depends, STREAM_DEPENDS, TTT, 0,
                                      0) OPT_ENTRY(stream_depends_e,
                                                   STREAM_DEPENDS_E, TTT, 0, 0)
#endif
#if LCURL_CURL_VER_GE(7, 56, 0)
                                OPT_ENTRY(mimepost, MIMEPOST, TTT, 0, 0)
#endif
...
#endif
#if LCURL_CURL_VER_GE(7, 64, 0)
                                        OPT_ENTRY(trailerfunction,
                                                  TRAILERFUNCTION, TTT, 0, 0)
#endif
#if LCURL_CURL_VER_GE(7, 74, 0) && LCURL_USE_HSTS
                                            OPT_ENTRY(hstsreadfunction,
                                                      HSTSREADFUNCTION, TTT, 0,
                                                      0)
                                                OPT_ENTRY(hstswritefunction,
                                                          HSTSWRITEFUNCTION,
                                                          TTT, 0, 0)
#endif
#undef OPT_ENTRY
...
#define OPT_ENTRY(N) {#N, CURL##N},
    ...
    OPT_ENTRY(INFO_TEXT) OPT_ENTRY(INFO_HEADER_IN) OPT_ENTRY(INFO_HEADER_OUT)
        OPT_ENTRY(INFO_DATA_IN) OPT_ENTRY(INFO_DATA_OUT)
            OPT_ENTRY(INFO_SSL_DATA_OUT) OPT_ENTRY(INFO_SSL_DATA_IN)

// File types for CURL_CHUNK_BGN_FUNCTION
#if LCURL_CURL_VER_GE(7, 21, 0)
                OPT_ENTRY(FILETYPE_DEVICE_BLOCK) OPT_ENTRY(FILETYPE_DEVICE_CHAR)
                    OPT_ENTRY(FILETYPE_DIRECTORY) OPT_ENTRY(FILETYPE_DOOR)
                        OPT_ENTRY(FILETYPE_FILE) OPT_ENTRY(FILETYPE_NAMEDPIPE)
...
void lcurl_easy_initlib(lua_State *L, int nup) {
  ...
  */
  ...
  LCURL_ASSERT_SAME_OFFSET(lcurl_easy_t, rbuffer, lcurl_hpost_stream_t,
                           rbuffer);
  LCURL_ASSERT_SAME_FIELD_SIZE(lcurl_easy_t, rbuffer, lcurl_hpost_stream_t,
                               rbuffer);
