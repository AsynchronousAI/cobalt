...
const char* get_ios_version() {
  ...
  id device =
      objc_msgSend((id)UIDeviceClass, sel_registerName("currentDevice"));
  if (device == NULL) {
    return "Unknown";
  }
  ...
  const char* version = ((const char*)objc_msgSend(
      systemVersion, sel_registerName("UTF8String")));
  if (version == NULL) {
    return "Unknown";
  }
  return version;
...
static int get_device_info(lua_State* L) {
  ...
  if (disabled == 1) {
    return luaL_error(L,
                      "Device information is not accessible on this platform");
  }
  ...
  lua_pushstring(L, "Windows");
  lua_settable(L, -3);
  lua_pushstring(L, "scriptmemoryint");  // How much memory is cobalt using
...
static const struct luaL_Reg device_lib[] = {
    {"info", get_device_info}, {"specs", get_device_specs}, {NULL, NULL}};
...
#elif TARGET_OS_MAC  // Note for language hackers: TARGET_OS_MAC only work when
