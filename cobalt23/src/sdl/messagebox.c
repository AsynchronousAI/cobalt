...
static void freemessageboxbuttondata(lua_State *L,
                                     SDL_MessageBoxButtonData *data) {
  if (data && data->text) {
    Free(L, (void *)data->text);
    data->text = NULL;
  }
}

static int checkmessageboxbuttondata(lua_State *L, int arg,
                                     SDL_MessageBoxButtonData *data) {
...
static void freemessageboxdata(lua_State *L, messagebox_t *data) {
  ...
  if (data->buttons) {
    for (int i = 0; i < data->numbuttons; i++)
      freemessageboxbuttondata(L,
                               (SDL_MessageBoxButtonData *)&data->buttons[i]);
  }
...
static messagebox_t *checkmessageboxdata(lua_State *L, int arg) {
  ...
  if (!lua_istable(L, arg)) {
    Free(L, messagebox);
    argerror(L, arg, ERR_TABLE);
    return NULL;
  }
  lua_getfield(L, arg, "flags");  // messageboxflags
  if (!lua_isinteger(L, -1)) {
    Free(L, messagebox);
    argerror(L, arg, ERR_VALUE);
    return NULL;
  }
  ...
  messagebox->buttons =
      MallocNoErr(L, sizeof(SDL_MessageBoxButtonData) * messagebox->numbuttons);
  ...
  for (int i = 0; i < messagebox->numbuttons; i++) {
    int err;
    lua_rawgeti(L, -1, i + 1);
    err = checkmessageboxbuttondata(
        L, -1, (SDL_MessageBoxButtonData *)&messagebox->buttons[i]);
    if (err) {
      Free(L, messagebox);
      argerror(L, arg, ERR_VALUE);
      return NULL;
  ...
  lua_pop(L, 1);
  // data->colorScheme = NULL by default (it can be set or deleted with the
  // set_color_scheme method)
  return messagebox;
}
...
static int SetColorScheme(lua_State *L) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    SDL_MessageBoxColorScheme *scheme =
        (SDL_MessageBoxColorScheme *)messagebox->colorScheme;
