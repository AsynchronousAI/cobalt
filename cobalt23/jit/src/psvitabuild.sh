...
%LJCOMPILE% host\minicobalt.c
@if errorlevel 1 goto :BAD
%LJLINK% /out:minicobalt.exe minicobalt.obj
@if errorlevel 1 goto :BAD
if exist minicobalt.exe.manifest^
  %LJMT% -manifest minicobalt.exe.manifest -outputresource:minicobalt.exe
...
minicobalt %DASM% -LN %DASMFLAGS% -o host\buildvm_arch.h vm_arm.dasc
@if errorlevel 1 goto :BAD
...
@if errorlevel 1 goto :BAD
buildvm -m vmdef -o jit\vmdef.cobalt %ALL_LIB%
@if errorlevel 1 goto :BAD
...
@del *.o *.obj *.manifest minicobalt.exe buildvm.exe
@echo.
