...
luaR_result luaR_findglobal(const char *name, lu_byte *ptype) {
  unsigned i;
  *ptype = LUA_TNIL;
  if (strlen(name) > LUA_MAX_ROTABLE_NAME) return 0;
...
static luaR_result luaR_findkey(const void *where, const char *key, int type,
                                int *found) {
  const char *pname;
  ...
  *found = 0;
  if (!where) return 0;
  while (1) {
    if (!(pname = isfunction ? pf->name : pv->name)) break;
    if (!strcmp(pname, key)) {
      *found = 1;
      return isfunction ? (luaR_result)(size_t)pf->func
                        : (luaR_result)pv->value;
    }
    pf++;
    pv++;
  }
