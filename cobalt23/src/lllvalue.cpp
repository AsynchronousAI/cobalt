...
Value::Value(CompilerState& cs) : cs_(cs) {}
...
Constant::Constant(CompilerState& cs, int arg)
    : Value(cs), tvalue_(cs.proto_->k + arg) {}

llvm::Value* Constant::GetTag() { return cs_.MakeInt(rttype(tvalue_)); }
...
llvm::Value* Constant::GetBoolean() { return cs_.MakeInt(bvalue(tvalue_)); }
...
MutableValue::MutableValue(CompilerState& cs) : Value(cs) {}
...
void MutableValue::SetTagK(int tag) { SetTag(cs_.MakeInt(tag)); }
...
llvm::Value* MutableValue::GetField(Field field) {
  std::vector<llvm::Value*> indices = {cs_.MakeInt(0), cs_.MakeInt((int)field)};
...
Register::Register(CompilerState& cs, int arg)
    : MutableValue(cs), arg_(arg), tvalue_(nullptr) {}
...
RTRegister::RTRegister(CompilerState& cs, llvm::Value* tvalue)
    : MutableValue(cs), tvalue_(tvalue) {}

llvm::Value* RTRegister::GetTValue() { return tvalue_; }

Upvalue::Upvalue(CompilerState& cs, int arg)
    : MutableValue(cs), arg_(arg), upval_(nullptr) {}

void Upvalue::ReloadTValue() {
  auto upvalptr =
      cs_.B_.CreateGEP(cs_.values_.upvals, cs_.MakeInt(arg_), "upval.ptr");
  upval_ = cs_.B_.CreateLoad(upvalptr, "upval");
  tvalue_ = cs_.LoadField(upval_, cs_.rt_.GetType("TValue"), offsetof(UpVal, v),
                          "upval.v");
}

llvm::Value* Upvalue::GetUpVal() { return upval_; }

llvm::Value* Upvalue::GetTValue() { return tvalue_; }

Stack::Stack(CompilerState& cs) : cs_(cs) {
  for (int i = 0; i < cs.proto_->sizek; ++i) k_.emplace_back(cs, i);
  for (int i = 0; i < cs.proto_->maxstacksize + 1; ++i) r_.emplace_back(cs, i);
  for (int i = 0; i < cs.proto_->sizeupvalues; ++i) u_.emplace_back(cs, i);
}
...
void Stack::InitValues() {
  for (auto& r : r_) r.Init();
}

void Stack::Update() {
  cs_.UpdateBase();
  for (auto& r : r_) r.ReloadTValue();
}
