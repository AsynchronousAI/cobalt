...
/* API Notes.
 * 1. Each mime can be root or child. If mime is a child (subpart) then curl
 * free it when parent mime is freed or when remove this part from parent. There
 * no way reuse same mime. Its not clear is it possible use mime created by one
 * easy handle when do preform in another. `m=e1:mime() e2:setopt_httpmime(m)
 * e1:close() e2:perform()`
 *
...
static void lcurl_mime_part_remove_subparts(lua_State *L, lcurl_mime_part_t *p,
                                            int free_it);

static lcurl_mime_t *lcurl_mime_part_get_subparts(lua_State *L,
...
static void lcurl_mime_part_remove_subparts(lua_State *L, lcurl_mime_part_t *p,
  ...
  if (sub) {
    ...
    /* if set `sub->parent = NULL` then gc for mime will try free
     * curl_mime_free. */
...
#define IS_NILORSTR(L, i) \
  (lua_type(L, i) == LUA_TSTRING) || (lua_type(L, i) == LUA_TNIL)
#define IS_TABLE(L, i) lua_type(L, i) == LUA_TTABLE
#define IS_FALSE(L, i)                                            \
  ((lua_type(L, i) == LUA_TBOOLEAN) && (!lua_toboolean(L, i))) || \
      lutil_is_null(L, i)
...
static lcurl_mime_part_t *lcurl_mime_parts_append(lcurl_mime_t *m,
                                                  lcurl_mime_part_t *p) {
  if (!m->parts)
    m->parts = p;
  else {
...
static lcurl_mime_part_t *lcurl_mime_parts_find(lcurl_mime_t *m,
                                                lcurl_mime_part_t *p) {
  lcurl_mime_part_t *ptr;
...
int lcurl_mime_create(lua_State *L, int error_mode) {
  ...
  if (!p->mime)
    return lcurl_fail_ex(L, error_mode, LCURL_ERROR_EASY, CURLE_FAILED_INIT);
...
lcurl_mime_part_t *lcurl_getmimepart_at(lua_State *L, int i) {
  lcurl_mime_part_t *p =
      (lcurl_mime_part_t *)lutil_checkudatap(L, i, LCURL_MIME_PART);
...
static int lcurl_mime_part_to_s(lua_State *L) {
  lcurl_mime_part_t *p =
      (lcurl_mime_part_t *)lutil_checkudatap(L, 1, LCURL_MIME_PART);
  luaL_argcheck(L, p != NULL, 1, LCURL_MIME_PART_NAME " object expected");

  lua_pushfstring(L, LCURL_MIME_PART_NAME " (%p)%s", (void *)p,
                  p->part ? "" : " (freed)");
  return 1;
}
...
#define UNSET_VALUE (const char *)-1
  ...
  if (mime_fname) {
    ret = curl_mime_filename(p->part,
                             mime_fname == UNSET_VALUE ? NULL : mime_fname);
    if (ret != CURLE_OK) {
      return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_EASY, ret);
    }
  }
...
static int lcurl_mime_part_subparts(lua_State *L) {
  ...
  /* if we already have one subpart then libcurl free it so we can not use any
   * references to it */
...
static int lcurl_pushvalues(lua_State *L, int nup) {
  ...
  if (nup > 0) {
    ...
    for (i = b; i <= e; ++i) lua_pushvalue(L, i);
  }
