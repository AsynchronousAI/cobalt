...
static int AddVersions(lua_State *L) {
  ...
  do {
    ...
    if (!((ver->major == 2) && (ver->minor >= 6)))
      return luaL_error(L,
                        "SDL port requires SDL_image >= 2.6.0 (found %d.%d.%d)",
                        ver->major, ver->minor, ver->patch);
    lua_pushstring(L, "_SDL_IMG_VERSION");
    lua_pushfstring(L, "SDL_image %d.%d.%d", ver->major, ver->minor,
                    ver->patch);
    lua_settable(L, -3);
  } while (0);
  ...
  do {
    ...
    if (!((ver->major == 2) && (ver->minor >= 20)))
      return luaL_error(L,
                        "SDL port requires SDL_ttf >= 2.20.0 (found %d.%d.%d)",
                        ver->major, ver->minor, ver->patch);
  ...
  do {
    ...
    if (!((ver->major == 2) && (ver->minor >= 6)))
      return luaL_error(L,
                        "SDL port requires SDL_mixer >= 2.6.0 (found %d.%d.%d)",
                        ver->major, ver->minor, ver->patch);
    lua_pushstring(L, "_SDL_MIX_VERSION");
    lua_pushfstring(L, "SDL_mixer %d.%d.%d", ver->major, ver->minor,
                    ver->patch);
    lua_settable(L, -3);
  } while (0);
