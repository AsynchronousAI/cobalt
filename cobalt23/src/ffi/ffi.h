...
static void lua_callk(lua_State* L, int nargs, int nresults, int ctx,
                      lua_CFunction k) {
...
#define ARCH_X64
#elif defined __arm__ || defined __ARM__ || defined ARM || defined __ARM || \
...
#define IS_COMPLEX(type) \
  ((type) == COMPLEX_FLOAT_TYPE || (type) == COMPLEX_DOUBLE_TYPE)
...
struct ctype {
  ...
  size_t offset;
  unsigned
      align_mask : 4; /* as (align bytes - 1) eg 7 gives 8 byte alignment */
  unsigned pointers : POINTER_BITS; /* number of dereferences to get to the base
                                       type including +1 for arrays */
  unsigned const_mask : POINTER_MAX +
                        1; /* const pointer mask, LSB is current pointer, +1 for
                              the whether the base type is const */
  ...
  unsigned is_variable_array : 1; /* set for variable array types where we don't
                                     know the variable size yet */
  unsigned is_variable_struct : 1;
  unsigned variable_size_known : 1; /* used for variable structs after we know
                                       the variable size */
...
void* push_cdata(lua_State* L, int ct_usr,
                 const struct ctype* ct); /* called from asm */
...
void parse_argument(lua_State* L, struct parser* P, int ct_usr,
                    struct ctype* type, struct token* name,
...
void compile_function(lua_State* L, cfunction f, int ct_usr,
...
/* WARNING: assembly needs to be updated for prototype changes of these
 * functions */
...
/* these two will always push a value so that we can create structs/functions on
 * the fly */
